...
CBattleEntity* CEnmityContainer::GetHighestEnmity()
{
    ...
    if (m_ProvokeLockID != 0)
    {
        if (auto lockIt = m_EnmityList.find(m_ProvokeLockID); lockIt != m_EnmityList.end())
        {
            CBattleEntity* PLockedTarget = lockIt->second.PEnmityOwner;
            if (!PLockedTarget)
            {
                PLockedTarget = zoneutils::GetChar(lockIt->first);
            }

            if (PLockedTarget && !PLockedTarget->isDead() && PLockedTarget->getZone() == m_EnmityHolder->getZone() && PLockedTarget->PInstance == m_EnmityHolder->PInstance)
            {
                return PLockedTarget;
            }
        }
...
{
    // Sticky provoke: keep relative hate stable until the lock clears (death, zone, or another Provoke).
    if (m_ProvokeLockID != 0)
    {
        return;
    }
...
void CEnmityContainer::SetProvokeLock(CBattleEntity* PEntity)
{
    if (PEntity)
    {
        m_ProvokeLockID = PEntity->id;
    }
}

void CEnmityContainer::ClearProvokeLock(uint32 EntityID)
{
    if (EntityID == 0 || m_ProvokeLockID == EntityID)
    {
        m_ProvokeLockID = 0;
    }
}

uint32 CEnmityContainer::GetProvokeLockID() const
{
    return m_ProvokeLockID;
}
