...
#include "utils/market_lottery.h"
...
void GP_CLI_COMMAND_SHOP_SELL_SET::process(MapSession* PSession, CCharEntity* PChar) const
{
    ...
    const uint32 payout = market_lottery::playerVendorPayoutAfterTax(PChar, cost);
    charutils::UpdateItem(PChar, LOC_INVENTORY, 0, payout);
    // TODO: Don't pass around Scheduler& through PSession
    auditSale(*PSession->scheduler, PChar, itemId, quantity, basePrice);
    ShowInfo("GP_CLI_COMMAND_SHOP_SELL_SET: Player '%s' sold %u of itemID %u (Vendor gross: %u gil, paid: %u gil) ", PChar->getName(), quantity, itemId, cost, payout);
