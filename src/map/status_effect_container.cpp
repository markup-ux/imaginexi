...
namespace
{
// Imagine XI: only Signet (and Abyssea Visitant) survive logout / reconnect.
bool isLoginPersistedEffect(EFFECT effectId)
{
    return effectId == EFFECT_SIGNET || effectId == EFFECT_VISITANT;
}
} // namespace
...
void CStatusEffectContainer::LoadStatusEffects()
{
    ...
    FOR_DB_MULTIPLE_RESULTS(rset)
    {
        ...
        if (!isLoginPersistedEffect(effectID))
        {
            continue;
        }
...
void CStatusEffectContainer::SaveStatusEffects(bool logout)
{
    ...
    for (CStatusEffect* PStatusEffect : m_StatusEffectSet)
    {
        ...
        if (!isLoginPersistedEffect(PStatusEffect->GetStatusID()))
        {
            continue;
        }
...
void CStatusEffectContainer::TickRegen(timer::time_point tick)
{
    ...
    if (!m_POwner->isDead())
    {
        ...
        // Disease / Plague / Zombie: suppress passive HP regen from the REGEN mod.
        if (m_POwner->StatusEffectContainer->HasStatusEffect({ EFFECT_DISEASE, EFFECT_PLAGUE, EFFECT_CURSE_II }))
        {
            regen = 0;
        }
        ...
        if (poison)
        {
            ...
            if (damage > 0)
            {
                ...
                battleutils::OnPlayerDoTPoisonTick(m_POwner);
