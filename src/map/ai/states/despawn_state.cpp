...
, despawnTime_(timer::now() + (instantDespawn ? 0s : 3s))
{
    ...
    {
        const timer::duration adjusted = PMob->GetCampHeatAdjustedRespawnTime();
        if (timer::count_seconds(adjusted) == timer::count_seconds(PMob->m_RespawnTime))
        {
            PMob->loc.zone->spawnHandler()->registerForRespawn(PMob);
        }
        else
        {
            PMob->loc.zone->spawnHandler()->registerForRespawn(PMob, adjusted);
        }
    }
}
