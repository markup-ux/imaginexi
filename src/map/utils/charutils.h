...
void GrantLevelEligibleSpells(CCharEntity* PChar, bool suppressLearnMessages = false);
void LoadInventory(CCharEntity* PChar);
// If inventory slot 0 has no gil currency (e.g. legacy DB or skipped 0xFFFF/0 rows), create a 0-gil stack and persist it.
...
// True if the kill counts for regimes/RoE-style credit (in range, not CFH, mob not TooWeak vs mob exp check, level sync rules).
bool HasKillCredit(CCharEntity* PChar, CMobEntity* PMob, float range = 100.0f);
...
void  BuildingCharAbilityTable(CCharEntity* PChar, bool announceNewAbilities = false);
void  BuildingCharTraitsTable(CCharEntity* PChar, bool announceNewTraits = false);
...
int32 addSpell(CCharEntity* PChar, uint16 SpellID, bool showLearnMessage = false); // add a spell
...
bool  hasDualWieldAbility(CCharEntity* PChar);      // trait list, trait bitmask, or DUAL_WIELD mod
...
// Return client toward PlayOnline / character select (GP_GAME_LOGOUT_STATE::POLEXIT).
...
// Preview race: model id for equipped item on a body of previewRace (uses item_basic.subid + item_mods race when needed).
