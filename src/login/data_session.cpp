    ...
    switch (code)
    {
        case 0xA1: // 161
        {
            ...
            if (session.accountID == recievedAcccountID)
            {
                ...
                const auto rset1 = db::preparedStmt("SELECT chars.charid, chars.charname, chars.pos_zone, chars.pos_prevzone, char_stats.mjob,"
                                                    "char_look.race, char_look.face, char_look.head, char_look.body, char_look.hands, char_look.legs, char_look.feet, char_look.main, char_look.sub,"
                                                    "char_jobs.war, char_jobs.mnk, char_jobs.whm, char_jobs.blm, char_jobs.rdm, char_jobs.thf, char_jobs.pld, char_jobs.drk, char_jobs.bst, char_jobs.brd, char_jobs.rng,"
                                                    "char_jobs.sam, char_jobs.nin, char_jobs.drg, char_jobs.smn, char_jobs.blu, char_jobs.cor, char_jobs.pup, char_jobs.dnc, char_jobs.sch, char_jobs.geo, char_jobs.run, "
                                                    "chars.gmlevel, chars.nation, char_look.size, char_stats.sjob "
                                                    "FROM chars "
        ...
        case 0xA2: // 162 : "Notifying lobby of current selections" pt 2
        {
            ...
            if (auto viewSession = session.view_session.get())
            {
                ...
                session.justCreatedNewChar = false; // +6 key tweak applies only once; do not leak to later logins
