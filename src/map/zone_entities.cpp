...
void CZoneEntities::DecreaseZoneCounter(CCharEntity* PChar)
{
    ...
    // Remove pets (supports dual pets: process until none remain)
    while (PChar->HasAnyPet())
    {
        CBattleEntity* rawPet = PChar->PPet;
        if (rawPet == nullptr)
        {
            break;
        }

        if (rawPet->objtype == TYPE_PET)
        {
            charutils::BuildingCharPetAbilityTable(PChar, static_cast<CPetEntity*>(rawPet), 0); // blank the pet commands
        }
        else
        {
        ...
        if (rawPet->isCharmed)
        {
            petutils::DespawnPet(PChar, rawPet);
            continue;
        }

        if (rawPet->objtype == TYPE_PET)
        {
            rawPet->status = STATUS_TYPE::DISAPPEAR;
            if (static_cast<CPetEntity*>(rawPet)->getPetType() == PET_TYPE::AVATAR)
            {
                PChar->setModifier(Mod::AVATAR_PERPETUATION, 0);
            }
        }

        if (rawPet->PMaster == PChar)
        {
            rawPet->PAI->Disengage();

            FOR_EACH_PAIR_CAST_SECOND(CCharEntity*, PCurrentChar, m_charList)
            {
                SpawnIDList_t::iterator itr = PCurrentChar->SpawnPETList.find(rawPet->id);

                if (itr != PCurrentChar->SpawnPETList.end())
                {
                    PCurrentChar->SpawnPETList.erase(itr);
                    PCurrentChar->updateEntityPacket(rawPet, ENTITY_DESPAWN, UPDATE_NONE);
                }
            }

            PChar->RemovePetPointer(rawPet);
        }
    }
                    ...
                    if (PEntity != PCurrentChar)
                    {
                            ...
                            (PEntity->objtype != TYPE_PC || static_cast<CCharEntity*>(PEntity)->m_moghouseID == PCurrentChar->m_moghouseID))
                        {
                                ...
                                packetType == 0x028) // Action packet
                            {
                                ...
                                // Dynamic entities (targid 0x700+) can fail spawn-list lookup for 0x028 even when
                                // the client already has the model (spawn 0x00E succeeded). Without the packet,
                                // spell start/finish animations never play (e.g. scripted Raise from Lua).
