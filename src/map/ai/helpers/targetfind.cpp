...
namespace
{
    // Party AoE buffs (Accession, Curaga, etc.): include avatars, trusts, jug pets, and charmed mobs owned by a PC.
    bool isPartyAoEAllyPet(CBattleEntity* PTarget)
    {
        if (PTarget == nullptr || PTarget->PMaster == nullptr || PTarget->PMaster->objtype != TYPE_PC)
        {
            return false;
        }
...
void CTargetFind::findWithinArea(CBattleEntity* PTarget, AOE_RADIUS radiusType, float radius, uint8 findFlags, uint16 targetFlags)
{
    ...
    // Include party members' pets when PETS_CAN_AOE_BUFF (e.g. Accession on self still has caster==anchor TYPE_PC).
...
bool CTargetFind::validEntity(CBattleEntity* PTarget)
{
    ...
    else
    {
        ...
        if (PTarget->PMaster != nullptr)
        {
            ...
            {
                return isPartyAoEAllyPet(PTarget);
            }
        }
    }
