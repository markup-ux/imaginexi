...
bool CMagicState::HasCost()
{
    ...
    {
        // Ninjutsu does not require or consume inventory ninja tools (server QoL).
        return true;
    }
    // check has mp available
    if (!battleutils::CanAffordSpell(m_PEntity, GetSpell(), m_flags))
    {
...
void CMagicState::SpendCost()
{
    if (m_PSpell->getSpellGroup() == SPELLGROUP_NINJUTSU)
    {
        // No tool consumption; see HasCost().
        return;
    }
    if (m_PSpell->hasMPCost() && !m_PEntity->StatusEffectContainer->HasStatusEffect(EFFECT_MANAFONT) && !(m_flags & MAGICFLAGS_IGNORE_MP))
    {
