...
void MapEngine::onPixieRescue(std::vector<std::string>& inputs) const
{
    auto dispatchFunc = lua["xi"]["pixieRescue"]["dispatchToPlayer"];
    if (!dispatchFunc.valid())
    {
        fmt::print("> pixierescue unavailable: xi.pixieRescue.dispatchToPlayer not loaded\n");
        return;
    }

    uint32 deadFound       = 0;
    uint32 dispatched      = 0;
    uint32 skippedReraise  = 0;
    uint32 skippedCooldown = 0;
    uint32 skippedOther    = 0;
        ...
        [&](CZone* PZone)
        {
            PZone->ForEachChar(
                [&](CCharEntity* PChar)
                {
                    ...
                    sol::protected_function_result result = dispatchFunc(PChar, lua.create_table_with(
                                                                                    "ignoreCooldown", true,
                                                                                    "ignoreReraise", false));
                    ...
                    auto [ok, reason] = result.get<std::tuple<bool, std::string>>();

                    if (ok)
                    {
                        ++dispatched;
                    }
                    else if (reason == "reraise")
                    {
                        ++skippedReraise;
                    }
    ...
    fmt::print("> pixierescue: dead={} dispatched={} skipped_reraise={} skipped_cooldown={} skipped_other={}\n",
               deadFound, dispatched, skippedReraise, skippedCooldown, skippedOther);
}
