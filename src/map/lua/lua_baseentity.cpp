...
/************************************************************************
 *  Function: polExit()
 *  Purpose : Sends GP_GAME_LOGOUT_STATE::POLEXIT (toward PlayOnline / char select).
 *  Example : player:polExit()
 ************************************************************************/

void CLuaBaseEntity::polExit()
{
    if (auto* PChar = dynamic_cast<CCharEntity*>(m_PBaseEntity))
    {
        charutils::PolExit(PChar);
    }
}
...
{
    // Lua passes xi.slot.* as numbers (often lua_Number); is<uint8>() is often false. NPCs need the
    // equipment-slot path when a numeric slot is provided, or setModelId overwrites the whole entity.
    const bool numericSlot = slotObj.valid() && slotObj.get_type() == sol::type::number;
    const SLOTTYPE slot    = numericSlot ? static_cast<SLOTTYPE>(slotObj.as<int>()) : SLOT_MAIN;

    if (m_PBaseEntity->objtype == TYPE_PC || numericSlot)
    {
        switch (slot)
        {
            case SLOT_MAIN:
 ...
 *  Example : npc:setLook({ race = xi.race.HUME_M, face = 1, size = 1 })
 *  Note    : Only for equipped NPCs that dynamically change their race/face.
 *            Optional size (0-2) sets modelSize / GraphSize (PC S/M/L) for player-like previews.
 ************************************************************************/
void CLuaBaseEntity::setLook(const sol::table& look)
{
    if (auto* PNpc = dynamic_cast<CNpcEntity*>(m_PBaseEntity))
    {
        ...
        if (look["size"].valid())
        {
            const auto sz = look.get<uint32>("size");
            if (sz <= static_cast<uint32>(CharSize::Large))
            {
                PNpc->modelSize = static_cast<uint8>(sz);
            }
        }
        return;
    }
...
/************************************************************************
 *  Function: getAppearancePreviewModelIds()
 *  Purpose : Equipment model IDs as they should appear on previewRace (armor variants via item_basic.subid).
 *  Note    : For race-exclusive gear, resolves another item with same subid/ilevel/slot valid for previewRace when possible.
 ************************************************************************/
auto CLuaBaseEntity::getAppearancePreviewModelIds(uint8 previewRace) -> sol::table
{
    auto table = lua.create_table();
    if (m_PBaseEntity->objtype != TYPE_PC)
    {
        ShowWarning("getAppearancePreviewModelIds: caller is not a player (%s).", m_PBaseEntity->getName());
        table["head"]  = 0;
        table["body"]  = 0;
        table["hands"] = 0;
        table["legs"]  = 0;
    ...
    table["head"]  = charutils::GetAppearanceEquipmentModelForRace(PChar, SLOT_HEAD, previewRace);
    table["body"]  = charutils::GetAppearanceEquipmentModelForRace(PChar, SLOT_BODY, previewRace);
    table["hands"] = charutils::GetAppearanceEquipmentModelForRace(PChar, SLOT_HANDS, previewRace);
    table["legs"]  = charutils::GetAppearanceEquipmentModelForRace(PChar, SLOT_LEGS, previewRace);
    table["feet"]  = charutils::GetAppearanceEquipmentModelForRace(PChar, SLOT_FEET, previewRace);
    table["main"]  = charutils::GetAppearanceEquipmentModelForRace(PChar, SLOT_MAIN, previewRace);
    table["sub"]   = charutils::GetAppearanceEquipmentModelForRace(PChar, SLOT_SUB, previewRace);
    return table;
}
...
void CLuaBaseEntity::changeJob(uint8 newJob)
{
    if (auto* PChar = dynamic_cast<CCharEntity*>(m_PBaseEntity))
    {
        ...
        charutils::GrantLevelEligibleSpells(PChar);
    }
...
uint8 CLuaBaseEntity::levelRestriction(const sol::object& level)
{
    ...
    if ((level != sol::lua_nil) && level.is<int>())
    {
        ...
        uint8        NewMLevel   = 0;
        uint8 const  realMLevel  = PChar->jobs.job[PChar->GetMJob()];

        if (PChar->m_LevelRestriction != 0)
        {
            uint8 const serverMax    = settings::get<uint8>("main.MAX_LEVEL");
            uint8 const restrictedLv = std::min<uint8>(static_cast<uint8>(PChar->m_LevelRestriction), serverMax);

            // Party level sync: match designee level (sync up or down). Other restrictions cap downward only.
            if (PChar->StatusEffectContainer->HasStatusEffect(EFFECT_LEVEL_SYNC))
            {
                NewMLevel = std::max<uint8>(1, restrictedLv);
            }
            else
            {
                NewMLevel = std::min<uint8>(restrictedLv, realMLevel);
            }
        }
        else
        {
            NewMLevel = realMLevel;
        }
...
/************************************************************************
 *                                                                       *
 *  Function: getExpChainCount()                                         *
 *  Purpose : Returns active EXP chain count                             *
 *  Example : player:getExpChainCount()                                  *
 *                                                                       *
 ************************************************************************/

uint16 CLuaBaseEntity::getExpChainCount()
{
    if (m_PBaseEntity->objtype == TYPE_PC)
    {
        auto* PChar = static_cast<CCharEntity*>(m_PBaseEntity);
        if (PChar->expChain.chainTime > timer::now())
        {
            return PChar->expChain.chainNumber;
        }
    }
...
void CLuaBaseEntity::setTP(int16 value)
{
    ...
    {
        uint16 const prevTp = PBattle->health.tp;
        PBattle->health.tp  = static_cast<uint16>(std::clamp(static_cast<int32>(value), 0, 3000));
        PBattle->updatemask |= UPDATE_HP;

        if (PBattle->objtype == TYPE_PC)
        {
            battleutils::OnPlayerTpChanged(PBattle, prevTp);
        }
    }
}
...
void CLuaBaseEntity::capSkill(uint8 skill)
{
    if (skill < MAX_SKILLTYPE)
    {
        ...
        */
        uint16 cap = charutils::GetCombatMagicSkillCap(PChar, static_cast<SKILLTYPE>(skill));
        if (cap == 0)
        {
            cap = battleutils::GetMaxSkill(static_cast<SKILLTYPE>(skill), PChar->GetMJob(), PChar->GetMLevel());
        }

        uint16 maxSkill                   = cap * 10;
...
/************************************************************************
 *  Function: refreshCharSkills()
 *  Purpose : Rebuilds and sends the character skills menu packet
 *  Example : player:refreshCharSkills()
 *  Notes   :
 ************************************************************************/

void CLuaBaseEntity::refreshCharSkills() const
{
    if (auto* PChar = dynamic_cast<CCharEntity*>(m_PBaseEntity))
    {
        charutils::BuildingCharSkillsTable(PChar);
...
void CLuaBaseEntity::addSpell(uint16 spellID, const sol::optional<sol::table>& paramTable)
{
    ...
    // Add spell to player's active spell list (named learn message when not silent)
    if (charutils::addSpell(PChar, spellID, !silentLog))
    {
...
bool CLuaBaseEntity::checkKillCredit(CLuaBaseEntity* PLuaBaseEntity, const sol::object& minRange)
{
    ...
    return charutils::HasKillCredit(PChar, PMob, range);
}
...
/************************************************************************
 *  Function: reduceAllAbilityRecasts()
 *  Purpose : Shortens every active job-ability recast by N seconds.
 *  Notes   : Skips one-hour, level-96 two-hour, and mount timers. PCs only.
 ************************************************************************/

void CLuaBaseEntity::reduceAllAbilityRecasts(uint32 seconds)
{
    if (seconds == 0 || m_PBaseEntity->objtype != TYPE_PC)
    {
        return;
    }

    auto* PBattleEntity = dynamic_cast<CBattleEntity*>(m_PBaseEntity);
    if (PBattleEntity == nullptr || PBattleEntity->PRecastContainer == nullptr)
    {
        return;
    }

    PBattleEntity->PRecastContainer->ReduceRecasts(RECAST_ABILITY, std::chrono::seconds(seconds));
...
void CLuaBaseEntity::spawnPet(const sol::object& arg0)
{
    ...
    {
        auto* PChar = static_cast<CCharEntity*>(m_PBaseEntity);
        if (arg0 != sol::lua_nil && arg0.get_type() == sol::type::number)
        {
        ...
        {
            ShowError("CLuaBaseEntity::spawnPet : PetID must be a Lua number (got sol::type %d)", static_cast<int>(arg0.get_type()));
        }
    }
    else if (m_PBaseEntity->objtype == TYPE_MOB)
    {
 ...
 *           target:despawnPet(specificPet) -- dual-pet: dismiss one pet only
 *  Notes   : Upon death or dismissal or similar
 ************************************************************************/

void CLuaBaseEntity::despawnPet(sol::optional<CLuaBaseEntity*> maybePet)
{
    if (m_PBaseEntity->objtype == TYPE_NPC)
    {
        ShowWarning("Invalid Entity (NPC: %s) calling function.", m_PBaseEntity->getName());
    ...
    auto* PMaster = static_cast<CBattleEntity*>(m_PBaseEntity);

    if (maybePet && *maybePet != nullptr)
    {
        auto* petEntity = dynamic_cast<CBattleEntity*>((*maybePet)->m_PBaseEntity);
        if (petEntity == nullptr)
        {
            ShowWarning("despawnPet: invalid pet entity (%s).", m_PBaseEntity->getName());
            return;
        }
        bool owned = false;
        for (CBattleEntity* p : PMaster->GetAllPets())
        {
            if (p == petEntity)
            {
                owned = true;
        ...
        if (!owned)
        {
            ShowWarning("despawnPet: pet is not owned by %s.", m_PBaseEntity->getName());
            return;
        }
        petutils::DespawnPet(PMaster, petEntity);
        return;
    }

    if (PMaster->HasAnyPet())
    {
        petutils::DespawnPet(PMaster);
    }
}

/************************************************************************
 *  Function: despawnCharmedPet()
 *  Purpose : Removes a charmed mob pet (Beastmaster Leave) without dismissing avatars
 ************************************************************************/

void CLuaBaseEntity::despawnCharmedPet()
{
    if (m_PBaseEntity->objtype == TYPE_NPC)
    {
        ShowWarning("Invalid Entity (NPC: %s) calling function.", m_PBaseEntity->getName());
    ...
    auto* PBattle = static_cast<CBattleEntity*>(m_PBaseEntity);
    for (CBattleEntity* pet : PBattle->GetAllPets())
    {
        if (pet && pet->objtype == TYPE_MOB && pet->isCharmed)
        {
            petutils::DespawnPet(PBattle, pet);
            return;
        }
    }
}
...
bool CLuaBaseEntity::hasPet()
{
    ...
    for (CBattleEntity* pet : PTarget->GetAllPets())
    {
        if (pet && pet->status != STATUS_TYPE::DISAPPEAR)
        {
            return true;
        }
    }
 ...
 *  Purpose : Number of active battle pets (non-disappear)
 ************************************************************************/

uint8 CLuaBaseEntity::getPetCount()
{
    if (m_PBaseEntity->objtype != TYPE_PC)
    {
        ShowWarning("Invalid Entity (%s) calling getPetCount.", m_PBaseEntity->getName());
        return 0;
    }

    auto* PTarget = static_cast<CBattleEntity*>(m_PBaseEntity);
    if (PTarget->PPet != nullptr && PTarget->PPet->status != STATUS_TYPE::DISAPPEAR)
    {
        return 1;
    }
    return 0;
...
bool CLuaBaseEntity::hasJugPet()
{
    ...
    for (CBattleEntity* raw : PBattle->GetAllPets())
    {
        if (auto* PPet = dynamic_cast<CPetEntity*>(raw); PPet && PPet->status != STATUS_TYPE::DISAPPEAR)
        {
...
{
    return m_PBaseEntity->modelSize;
}
...
{
    m_PBaseEntity->modelSize = std::clamp<uint8>(newSize, 0, 3);
}
...
void CLuaBaseEntity::Register()
{
    ...
    SOL_REGISTER("refreshCharSkills", CLuaBaseEntity::refreshCharSkills);
    ...
    SOL_REGISTER("getPetCount", CLuaBaseEntity::getPetCount);
    ...
    SOL_REGISTER("polExit", CLuaBaseEntity::polExit);
