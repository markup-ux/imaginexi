...
auto CPetController::Tick(timer::time_point tick) -> Task<void>
{
    ...
    if (isPlayerPet)
    {
        ...
        {
            petutils::DespawnPet(PPet->PMaster, PPet);
            co_return;
        }
        ...
        if (PPetEntity && PPetEntity->isAlive() && PPetEntity->getPetType() == PET_TYPE::JUG_PET)
        {
            ...
            {
                petutils::DespawnPet(PPetEntity->PMaster, PPetEntity);
                co_return;
            }
        }
    }
...
auto CPetController::DoRoamTick(timer::time_point tick) -> Task<void>
{
    ...
    {
        position_t followAnchor = PPet->PMaster->loc.p;

        if (!PPet->PAI->PathFind->IsFollowingPath() ||
            distance(PPet->PAI->PathFind->GetDestination(), followAnchor) > 2.0f) // recalculate path only if anchor moves enough
        {
            if (!PPet->PAI->PathFind->PathAround(followAnchor, 2.0f, PATHFLAG_RUN | PATHFLAG_WALLHACK))
            {
                if (!PPet->PAI->PathFind->PathInRange(followAnchor, 2.0f, PATHFLAG_RUN | PATHFLAG_WALLHACK))
                {
