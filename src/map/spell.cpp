...
bool CanUseSpell(CBattleEntity* PCaster, CSpell* spell)
{
    ...
    switch (PCaster->objtype)
    {
        ...
        case TYPE_PC:
            // Imagine XI: Virus/Curse must be castable even if spell_list job bytes are stale in DB.
            if (spell->getID() == SpellID::Virus || spell->getID() == SpellID::Curse)
            {
                return true;
            }
...
std::string FormatScriptNameForDisplay(const std::string& scriptName)
{
    static const std::unordered_set<std::string> romanTokens = {
    ...
    auto appendWord = [&]()
    {
        ...
        else if (romanTokens.find(word) != romanTokens.end())
        {
            for (char ch : word)
            {
        ...
        {
            out.push_back(static_cast<char>(std::toupper(static_cast<unsigned char>(word.front()))));
            for (size_t i = 1; i < word.size(); ++i)
            {
                out.push_back(static_cast<char>(std::tolower(static_cast<unsigned char>(word[i]))));
            }
        }
        word.clear();
    };

    for (unsigned char uch : scriptName)
    {
        const char c = static_cast<char>(uch);
        if (c == '_' || std::isspace(uch))
        {
            appendWord();
        }
        else
