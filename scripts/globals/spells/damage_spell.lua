...
xi.spells.damage.useDamageSpell = function(caster, target, spell)
    ...
    local chargedNukeWillSpend = false

    if xi.magicTp.prepareChargedNuke then
        finalDamage, chargedNukeWillSpend = xi.magicTp.prepareChargedNuke(caster, target, spell, finalDamage, true)
    end

    -- Battle packet param (chat damage number) must not use enmity or a nil takeSpellDamage return.
    local messageDamage = finalDamage

    -- Handle Bind break and TP?
    local appliedDamage = target:takeSpellDamage(caster, spell, finalDamage, xi.attackType.MAGICAL, xi.damageType.ELEMENTAL + spellElement)
    appliedDamage         = tonumber(appliedDamage) or 0

    -- Handle Afflatus Misery.
    target:handleAfflatusMiseryDamage(appliedDamage)

    -- Handle Enmity.
    if xi.magicTp.commitChargedNuke then
        xi.magicTp.commitChargedNuke(caster, target, spell, appliedDamage, chargedNukeWillSpend)
    end

    local enmityBase   = appliedDamage > 0 and appliedDamage or messageDamage
    local enmityDamage = enmityBase
    if xi.magicTp.getChargedNukeEnmity then
        enmityDamage = xi.magicTp.getChargedNukeEnmity(enmityBase, chargedNukeWillSpend)
    end

    target:updateEnmityFromDamage(caster, enmityDamage)
    ...
    -- Battle log shows calculated spell damage, not HP-capped killing-blow amount.
    if appliedDamage > 0 then
        return messageDamage
    end

    -- Charged edge case: show amplified damage when takeSpellDamage reports 0.
    if chargedNukeWillSpend and messageDamage > 0 then
        return messageDamage
    end
