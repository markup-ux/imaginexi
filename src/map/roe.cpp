...
auto event(const ROE_EVENT eventID, CCharEntity* PChar, const RoeDatagramList& payload) -> bool
{
    ...
    // RoE mob-kill objectives: only credit when this player would get kill credit (mob not TooWeak, etc.).
    if (eventID == ROE_MOBKILL)
    {
        CMobEntity* PMob = nullptr;
        for (const auto& datagram : payload)
        {
            if (datagram.luaKey == "mob")
            {
                if (auto* mobPtr = std::get_if<CMobEntity*>(&datagram.data))
                {
                    PMob = *mobPtr;
                    break;
                }
        ...
        if (PMob != nullptr && !charutils::HasKillCredit(PChar, PMob))
        {
            return false;
        }
    }
...
void UpdateUnityTrust(CCharEntity* PChar, const bool sendUpdate)
{
    ...
    if (unityLeaderTrust > 0)
    {
        ...
        {
            charutils::addSpell(PChar, unityLeaderTrust, true);
