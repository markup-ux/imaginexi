...
void GP_CLI_COMMAND_LOGIN::process(MapSession* PSession, CCharEntity* PChar) const
{
    ...
    if (PSession->blowfish.status == BLOWFISH_PENDING_ZONE || PSession->blowfish.status == BLOWFISH_WAITING) // Call zone in, etc, only once.
    {
        ...
        {
            ShowWarningFmt("GP_CLI_COMMAND_LOGIN: invalid destination zone {} for `{}` — retargeting to home point",
                           destination, PChar->getName());
            ...
            uint16       homeZone = PChar->profile.home_point.destination;
            position_t homePos  = PChar->profile.home_point.p;

            if (homeZone == 0 || homeZone >= MAX_ZONEID || zoneutils::GetZone(homeZone) == nullptr)
            {
                ShowWarningFmt("GP_CLI_COMMAND_LOGIN: home zone {} invalid for `{}` — using Southern San d'Oria fallback",
                               homeZone, PChar->getName());
                homeZone = 230; // Southern San d'Oria
                homePos  = position_t(-100.0f, 1.0f, -40.0f, 0, 224);
                PChar->profile.home_point.destination = homeZone;
                PChar->profile.home_point.p           = homePos;
            }

            PChar->loc.destination = homeZone;
            PChar->loc.p           = homePos;
            PChar->loc.boundary    = 0;

            destination = homeZone;
            destZone    = zoneutils::GetZone(destination);
            if (destZone == nullptr)
            {
                ShowErrorFmt("GP_CLI_COMMAND_LOGIN: could not resolve any zone for `{}` after fallback (zone {})",
                             PChar->getName(), destination);
                return;
            }
