...
-- Reive scripts assume mobs like Knotted_Root (or barriers in Ra'Kaznar) exist in mob_spawn_points
-- so GetFirstID populated REIVE_MOB_OFFSET in each zone's IDs.lua. If SQL is stripped/minimal, offsets
-- are nil and this file would error before assigning zoneData, breaking xi.reives.setupZone on init.
local function reiveOffsetsReady(ids)
    return ids and ids.mob and ids.mob.REIVE_MOB_OFFSET ~= nil
end

if not (
    reiveOffsetsReady(ceizakBattlegroundsID) and
    reiveOffsetsReady(cirdasCavernsID) and
    reiveOffsetsReady(dhoGatesID) and
    reiveOffsetsReady(foretDeHennetielID) and
    reiveOffsetsReady(kamihrDriftsID) and
    reiveOffsetsReady(marjamiRavineID) and
    reiveOffsetsReady(mohGatesID) and
    reiveOffsetsReady(morimarBasaltFieldsID) and
    reiveOffsetsReady(outerRakaznarID) and
    reiveOffsetsReady(rakaznarInnerCourtID) and
    reiveOffsetsReady(sihGatesID) and
    reiveOffsetsReady(wohGatesID) and
    reiveOffsetsReady(yahseHuntingGroundsID) and
    reiveOffsetsReady(yorciaWealdID)
) then
    xi.reives.zoneData = {}
    return
end
