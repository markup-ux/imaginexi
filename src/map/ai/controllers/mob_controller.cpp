...
namespace
{
// Curated and battle-tested mobskill IDs to avoid animation/state desync loops.
// These are injected for HNMs in addition to their native skill list.
constexpr std::array<uint16, 5> kHnmApexMobSkills =
{
    717,  // venom breath (hnm-only list)
    724,  // evasion (hnm-only list)
    1744, // diamondhide (stoneskin; flat power in lua — not extra-strong for HNM)
    1017, // call_beast (opt-in pet link required; often skipped by skill check)
    1901, // activate (opt-in automaton link required; often skipped by skill check)
};

void AppendSkillIfValid(std::vector<uint16>& skillList, uint16 skillId)
{
    if (skillId == 0 || !battleutils::GetMobSkill(skillId))
    {
        return;
    }

    if (std::find(skillList.begin(), skillList.end(), skillId) == skillList.end())
    {
        skillList.push_back(skillId);
    }
}

void InjectConfiguredHnmApexSkills(CMobEntity* mob, std::vector<uint16>& skillList)
{
    const bool replaceDefaultPool = mob->GetLocalVar("[hnmApex]replaceDefault") == 1;

    if (!replaceDefaultPool)
    {
        for (const auto skillId : kHnmApexMobSkills)
        {
            AppendSkillIfValid(skillList, skillId);
        }
    }

    // Optional per-HNM override from Lua:
    // mob:setLocalVar("[hnmApex]skill1", 272) ... skill12
    // Any valid non-zero skill IDs here are appended to the curated safe pool.
    for (uint8 i = 1; i <= 12; ++i)
    {
        const std::string key = std::string("[hnmApex]skill") + std::to_string(i);
        const auto        raw = mob->GetLocalVar(key.c_str());
        if (raw > 0 && raw <= std::numeric_limits<uint16>::max())
        {
            AppendSkillIfValid(skillList, static_cast<uint16>(raw));
        }
    }
}
} // namespace
...
auto CMobController::TryDeaggro() -> bool
{
    ...
    const uint32 provokeLockId = PMob->PEnmityContainer ? PMob->PEnmityContainer->GetProvokeLockID() : 0;
    const bool   stickyProvoke = PTarget != nullptr && provokeLockId != 0 && PTarget->id == provokeLockId;

    bool targetInvalid = !PTarget || PTarget->isDead() || PTarget->isMounted() || PTarget->loc.zone->GetID() != PMob->loc.zone->GetID() ||
                         PMob->StatusEffectContainer->GetConfrontationEffect() != PTarget->StatusEffectContainer->GetConfrontationEffect() ||
                         PMob->allegiance == PTarget->allegiance || PMob->getBattleID() != PTarget->getBattleID();

    if (PTarget && !stickyProvoke)
    {
        targetInvalid = targetInvalid || CheckDetection(PTarget) || CheckHide(PTarget) || CheckLock(PTarget);
    }
...
auto CMobController::MobSkill(int listId) -> bool
{
    ...
    if (settings::get<bool>("map.HNM_APEX_SKILL_POOL_ENABLED") && (PMob->m_Type & MOBTYPE_NOTORIOUS))
    {
        InjectConfiguredHnmApexSkills(PMob, skillList);
    }
...
void CMobController::HandleEnmity()
{
    ...
    {
        const bool stickyProvokeBind = PMob->PEnmityContainer && PMob->PEnmityContainer->GetProvokeLockID() != 0;

        if (!stickyProvokeBind)
        {
