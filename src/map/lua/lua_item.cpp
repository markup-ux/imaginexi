...
uint32 CLuaItem::getJobs()
{
    if (const auto* PItem = dynamic_cast<const CItemEquipment*>(m_readItem))
    {
        return PItem->getJobs();
    }

    return 0;
}

uint16 CLuaItem::getEquipSlotId()
{
    if (const auto* PItem = dynamic_cast<const CItemEquipment*>(m_readItem))
    {
        return PItem->getEquipSlotId();
    }

    return 0;
}

uint8 CLuaItem::getSuperiorLevel()
{
    if (const auto* PItem = dynamic_cast<const CItemEquipment*>(m_readItem))
    {
        return const_cast<CItemEquipment*>(PItem)->getSuperiorLevel();
    }

    return 0;
}

uint16 CLuaItem::getWeaponDamage()
{
    if (const auto* PWeapon = dynamic_cast<const CItemWeapon*>(m_readItem))
    {
...
uint16 CLuaItem::getWeaponDelay()
{
    if (const auto* PWeapon = dynamic_cast<const CItemWeapon*>(m_readItem))
    {
        return PWeapon->getDelay();
    }
...
void CLuaItem::Register()
{
    ...
    SOL_REGISTER("getJobs", CLuaItem::getJobs);
    SOL_REGISTER("getEquipSlotId", CLuaItem::getEquipSlotId);
    SOL_REGISTER("getSuperiorLevel", CLuaItem::getSuperiorLevel);
    SOL_REGISTER("getWeaponDamage", CLuaItem::getWeaponDamage);
    SOL_REGISTER("getWeaponDelay", CLuaItem::getWeaponDelay);
