...
entity.onMobSpawn = function(mob)
    ...
    -- Optional family-native apex mode for Aspidochelone only.
    if xi.settings.map.ASPIDOCHELONE_FAMILY_NATIVE_APEX_MODE then
        mob:setLocalVar('[hnmApex]replaceDefault', 0)

        -- Adamantoise family list (mob_skill_lists id 2)
        mob:setLocalVar('[hnmApex]skill1', 804) -- Tortoise Song
        mob:setLocalVar('[hnmApex]skill2', 805) -- Head Butt (turtle)
        mob:setLocalVar('[hnmApex]skill3', 806) -- Tortoise Stomp
        mob:setLocalVar('[hnmApex]skill4', 807) -- Harden Shell
        mob:setLocalVar('[hnmApex]skill5', 808) -- Earth Breath
        mob:setLocalVar('[hnmApex]skill6', 809) -- Aqua Breath

        -- Optional conservative cross-family supplements.
        if xi.settings.map.ASPIDOCHELONE_SAFE_CROSS_FAMILY_APEX_MODE then
