...
void GP_CLI_COMMAND_ACTION::process(MapSession* PSession, CCharEntity* PChar) const
{
    ...
    switch (this->ActionID)
    {
        ...
        case GP_CLI_COMMAND_ACTION_ACTIONID::Weaponskill:
        {
            uint16 const wsid = static_cast<uint16>(this->Weaponskill.SkillId);

            if (!PChar->PAI->IsEngaged() && settings::get<bool>("map.PREVENT_UNENGAGED_WS") &&
                !battleutils::isMagicRangeStaffElementalWeaponskill(wsid))
            {
                PChar->pushPacket<GP_SERV_COMMAND_BATTLE_MESSAGE>(PChar, PChar, 0, 0, MsgBasic::UnableToUseWeaponskill);
                return;
            }

            if (!PChar->PAI->IsEngaged() && settings::get<bool>("map.AUTO_ENGAGE_ON_WEAPONSKILL") &&
                !battleutils::isMagicRangeStaffElementalWeaponskill(wsid))
            {
                PChar->PAI->Engage(this->ActIndex);
            }

            PChar->PAI->WeaponSkill(this->ActIndex, wsid);
        }
        break;
        case GP_CLI_COMMAND_ACTION_ACTIONID::JobAbility:
        {
        ...
        case GP_CLI_COMMAND_ACTION_ACTIONID::Mount:
        {
            ...
            else if (PChar->GetMLevel() < 10)
            {
                PChar->pushPacket<GP_SERV_COMMAND_BATTLE_MESSAGE>(PChar, PChar, 10, 0, MsgBasic::MountRequiredLevel);
            }
