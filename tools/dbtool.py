...
def import_file(file):
    ...
    # Feed SQL on stdin (not SOURCE inside -e): avoids Windows cmdline limits and
    # clients run with --sandbox, which blocks SOURCE.
    prefix = (
        "SET autocommit=0;\n"
        "SET unique_checks=0;\n"
    ...
    try:
        with open(file, "r", encoding="utf-8", errors="replace") as sql_f:
            script = prefix + sql_f.read() + suffix
    except OSError as e:
        print_red(f"Could not read SQL file {file}: {e}")
        preflight_exit()
    ...
    fetch_errors(f"stdin import of {file}", result)
