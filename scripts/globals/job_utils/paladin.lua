...
xi.job_utils.paladin.useHolyCircle = function(player, target, ability)
    -- Killer: power (+ potency). Affinity (damage vs. / DT from undead): subPower, applied in battleutils.
    -- https://www.bg-wiki.com/ffxi/Holy_Circle
    local function calculateHolyCircleEnlightParams(member)
        local vit = member:getStat(xi.mod.VIT)
        local enspellPower = math.min(60, math.max(8, math.floor(vit / 8) + 8))
        local enspellDuration = math.min(900, math.max(120, 120 + vit * 4))

        return enspellPower, enspellDuration
    end

    local function shouldApplyHolyCircleEnlight(member, enspellPower, enspellDuration)
        local existing = member:getStatusEffect(xi.effect.ENLIGHT)
        if existing == nil then
            return true
        end

        local newDurationMs = enspellDuration * 1000
        if existing:getPower() < enspellPower then
            return true
        end

        return existing:getPower() == enspellPower and existing:getTimeRemaining() < newDurationMs
    end

    local longCircleDurationS = 3600
    local duration            = longCircleDurationS + player:getMod(xi.mod.HOLY_CIRCLE_DURATION)
    local power               = 15
    local subPower            = 15
    ...
    if player:getMainJob() ~= xi.job.PLD then
        ...
        subPower = subPower + player:getJobPointLevel(xi.jp.HOLY_CIRCLE_EFFECT)
    end

    power = power + player:getMod(xi.mod.HOLY_CIRCLE_POTENCY)

    target:addStatusEffect(xi.effect.HOLY_CIRCLE, { power = power, subPower = subPower, duration = duration, origin = player })

    local enspellPower, enspellDuration = calculateHolyCircleEnlightParams(target)
    if shouldApplyHolyCircleEnlight(target, enspellPower, enspellDuration) then
        target:delStatusEffectSilent(xi.effect.ENLIGHT)
        target:addStatusEffect(xi.effect.ENLIGHT, { power = enspellPower, duration = enspellDuration, origin = player })
    end
