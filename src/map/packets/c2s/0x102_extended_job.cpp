        ...
        else
        {
            ...
            if (spellIndex != -1 && spellInQuestion != 0)
            {
                ...
                if (spell != nullptr)
                {
                    ...
                    const uint8 bluReq = spell->getJob(JOB_BLU);

                    const bool mainCanSet = PChar->GetMJob() == JOB_BLU && mLevel >= bluReq;
                    const bool subCanSet  = PChar->GetSJob() == JOB_BLU && sLevel >= bluReq;

                    if (!mainCanSet && !subCanSet)
                    {
