...
void LoadAutomaton(CCharEntity* PChar)
{
    ...
    // Legacy/imported chars may be missing char_pet; the SELECT would return no row, equipment stays
    // zeroed, and spawnPet remaps PETID 69 using frame 0 into jug pet 37 (no automaton on Activate).
    db::preparedStmt("INSERT IGNORE INTO char_pet (charid) VALUES (?)", PChar->id);
    ...
    if (rset && rset->rowsCount() && rset->next())
    {
        ...
        if (PChar->GetMJob() == JOB_PUP || PChar->GetSJob() == JOB_PUP)
        {
            ...
            bool appliedDefaultEquip = false;
            ...
            // setHead/setFrame no-op when unlocked_attachments does not include the equipped frame/head
            // (imported chars, bad unlock blob, etc.). m_Equip.Frame can stay 0 so Activate remaps to a jug pet.
            if (PChar->getAutomatonFrame() < AutomatonFrame::Harlequin || PChar->getAutomatonFrame() > AutomatonFrame::Stormwaker ||
                PChar->getAutomatonHead() < AutomatonHead::Harlequin || PChar->getAutomatonHead() > AutomatonHead::Spiritreaver)
            {
                PChar->setAutomatonHead(AutomatonHead::Harlequin);
                tempEquip.Head = AutomatonHead::Harlequin;
                PChar->setAutomatonFrame(AutomatonFrame::Harlequin);
