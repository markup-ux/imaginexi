...
--- Applies HP, wake, charged-cure effects, and enmity for white-magic heals (Cure/Curaga/Cura).
--- Works with Accession, Manifestation, Majesty, Divine Seal, and other pre-calc JA modifiers.
local function finishWhiteMagicHeal(caster, target, spell, final, fixedCE, fixedVE)
    if final <= 0 then
        return 0
    end

    local healAmount = final
    local willCharge = false

    if xi.magicTp and xi.magicTp.prepareChargedCure then
        healAmount, willCharge = xi.magicTp.prepareChargedCure(caster, target, spell, final)
    end
    ...
    -- Battle packet param (chat heal number) must not use enmity or a nil addHP return.
    -- Keep calculated heal for display; HP application may be lower when target is nearly full.
    local messageHeal = healAmount

    local appliedHeal = tonumber(target:addHP(healAmount)) or 0

    target:wakeUp()

    if xi.magicTp and xi.magicTp.applyChargedCureEffects then
        local ok, err = pcall(xi.magicTp.applyChargedCureEffects, caster, target, spell, healAmount, willCharge)
        if not ok then
            print(string.format('[magic] applyChargedCureEffects failed: %s', tostring(err)))
        end
    end

    local enmityAmount = healAmount
    if xi.magicTp and xi.magicTp.getChargedCureEnmity then
        enmityAmount = xi.magicTp.getChargedCureEnmity(caster, healAmount, willCharge)
    end

    if fixedCE and fixedVE then
        caster:updateEnmityFromCure(target, enmityAmount, fixedCE, fixedVE)
    else
        caster:updateEnmityFromCure(target, enmityAmount)
    end

    return math.floor(messageHeal)
end

--- Applies HP, wake, Imagine XI charged-cure effects, and enmity for single-target white cures.
function finishWhiteMagicCure(caster, target, spell, final, fixedCE, fixedVE)
    return finishWhiteMagicHeal(caster, target, spell, final, fixedCE, fixedVE)
end

--- Curaga/Cura helper: charged heal + AoE chat message.
function finishWhiteMagicCuraga(caster, target, spell, final, fixedCE, fixedVE)
    local healAmount = finishWhiteMagicHeal(caster, target, spell, final, fixedCE, fixedVE)
