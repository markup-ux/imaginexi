...
auto PetSkillDistanceCheck(CCharEntity* PChar, CBaseEntity* PTarget, const CAbility* PAbility) -> bool
{
    ...
    CBattleEntity* pet = nullptr;
    if (PPetSkill->isBloodPactRage() || PPetSkill->isBloodPactWard())
    {
        for (CBattleEntity* raw : PChar->GetAllPets())
        {
            if (auto* cp = dynamic_cast<CPetEntity*>(raw);
                cp && cp->getPetType() == PET_TYPE::AVATAR)
            {
                pet = cp;
                break;
            }
    ...
    else if (PPetSkill->getMobSkillID() > 0)
    {
        ...
        {
            if (auto* cp = dynamic_cast<CPetEntity*>(raw);
                cp && cp->getPetType() == PET_TYPE::JUG_PET)
            {
                pet = cp;
                break;
            }
        ...
        if (pet == nullptr)
        {
            for (CBattleEntity* raw : PChar->GetAllPets())
            {
                if (raw && raw->objtype == TYPE_MOB && raw->isCharmed)
                {
                    pet = raw;
                    break;
                }
    ...
    if (PPetSkill->isBloodPactRage() || PPetSkill->isBloodPactWard())
    {
        ...
        if (distance(pet->loc.p, PTarget->loc.p) > PPetSkill->getDistance() + pet->modelHitboxSize + PTarget->modelHitboxSize)
        {
    ...
    else if (PPetSkill->getMobSkillID() > 0)
    {
        ...
        if (distance(PChar->loc.p, pet->loc.p) > 4.0f + PChar->modelHitboxSize + pet->modelHitboxSize)
        {
        ...
        {
            if (auto* PPetTarget = pet->GetBattleTarget(); PPetTarget && distance(pet->loc.p, PPetTarget->loc.p) > PPetSkill->getDistance() + pet->modelHitboxSize + PPetTarget->modelHitboxSize)
            {
    ...
    if (PTarget)
    {
        if (m_PAbility->getValidTarget() & TARGET_ENEMY && PTarget->allegiance != m_PEntity->allegiance)
        {
            if (PTarget->objtype == TYPE_MOB && !(m_PAbility->getCE() == 0 && m_PAbility->getVE() == 0))
            {
                ...
                if (m_PAbility->getID() == ABILITY_PROVOKE || m_PAbility->getID() == ABILITY_ANIMATED_FLOURISH)
                {
                    mob->PEnmityContainer->SetProvokeLock(m_PEntity);
                }
