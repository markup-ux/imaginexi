...
{
    if (PMaster == nullptr || !PMaster->HasAnyPet() || PTarget == nullptr)
    {
        ShowWarning("petutils::AttackTarget() - Null master, pet, or target passed to function.");
        return;
    }

    for (CBattleEntity* PPet : PMaster->GetAllPets())
    {
...
{
    if (PMaster == nullptr || !PMaster->HasAnyPet())
    {
...
void CalculateWyvernStats(CBattleEntity* PMaster, CPetEntity* PPet)
{
    ...
    if (PMaster->objtype == TYPE_PC)
    {
        ...
        if (PMaster->GetMJob() == JOB_DRG || PMaster->GetSJob() == JOB_DRG)
        {
            PPet->addModifier(Mod::ACC, PMaster->getMod(Mod::PET_ACC_EVA));
            PPet->addModifier(Mod::EVA, PMaster->getMod(Mod::PET_ACC_EVA));
            PPet->addModifier(Mod::MACC, PMaster->getMod(Mod::PET_MACC_MEVA));
...
void SpawnPet(CBattleEntity* PMaster, uint32 PetID, bool spawningFromZone)
{
    ...
    CPetEntity* PPet = LoadPet(PMaster, PetID, spawningFromZone);
    if (PPet)
    {
...
void DetachPet(CBattleEntity* PMaster)
{
    ...
    DetachPet(PMaster, PMaster->PPet);
}

void DetachPet(CBattleEntity* PMaster, CBattleEntity* PPetToDetach)
{
    if (PMaster == nullptr || PPetToDetach == nullptr)
    {
        ShowWarning("PMaster or pet is null.");
        return;
    }
    ...
    if (PPetToDetach != PMaster->PPet)
    {
        ShowWarning("DetachPet: pet does not belong to master (%s)", PMaster->getName());
        return;
    }

    CCharEntity* PChar = static_cast<CCharEntity*>(PMaster);

    if (PPetToDetach->objtype == TYPE_MOB)
    {
        CMobEntity* PMob = static_cast<CMobEntity*>(PPetToDetach);
    ...
    else if (PPetToDetach->objtype == TYPE_PET)
    {
        if (!PPetToDetach->isDead())
        {
            PPetToDetach->Die();
        }
        CPetEntity* PPetEnt = static_cast<CPetEntity*>(PPetToDetach);
    ...
    PMaster->RemovePetPointer(PPetToDetach);
    if (PMaster->HasAnyPet())
    {
...
void DespawnPet(CBattleEntity* PMaster, CBattleEntity* PPetToDespawn)
{
    if (PMaster == nullptr || PPetToDespawn == nullptr)
    {
        ShowWarning("PMaster is null.");
        return;
    }

    petutils::DetachPet(PMaster, PPetToDespawn);
}
...
void DespawnPet(CBattleEntity* PMaster)
{
    ...
    petutils::DetachPet(PMaster, PMaster->PPet);
}

int16 PerpetuationCost(uint32 id, uint8 level)
{
    // ImagineXI custom rule: remove avatar perpetuation MP cost.
    // Keep signature intact for compatibility with existing callers.
    static_cast<void>(id);
...
CPetEntity* LoadPet(CBattleEntity* PMaster, uint32 PetID, bool spawningFromZone)
{
    if (PMaster == nullptr)
    {
        ShowWarning("PMaster is null.");
    ...
    else if (PPet->getPetType() == PET_TYPE::JUG_PET)
    {
        ...
        if (PMaster->objtype == TYPE_PC)
        {
            ...
            spawnLevel  = PChar->petZoningInfo.petLevel;
        }
...
bool IsTandemActive(CBattleEntity* PAttacker)
{
    ...
    */
    ...
    {
        for (CBattleEntity* pet : PAttacker->GetAllPets())
        {
            if (pet &&
                pet->PAI->IsEngaged() &&
                pet->GetBattleTarget() != nullptr &&
                pet->GetBattleTargetID() == PAttacker->GetBattleTargetID())
            {
                return true;
            }
        }
