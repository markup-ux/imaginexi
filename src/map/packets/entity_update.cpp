...
void CEntityUpdatePacket::updateWith(CBaseEntity* PEntity, ENTITYUPDATE type, uint8 updatemask)
{
    ...
    // Renamed dynamic NPCs with equipped (or chocobo) models need the extended 0x00E layout on every
    // send, not only ENTITY_SPAWN. The narrow renamed path below places packetName at 0x34, which
    // overlaps look_t equipment (head at 0x34+), stripping gear and breaking face/hair rendering.
    const bool renamedEquippedDynamic = PEntity->isRenamed &&
                                        (PEntity->look.size == MODEL_EQUIPPED || PEntity->look.size == MODEL_CHOCOBO) &&
                                        PEntity->targid >= 0x700;

    if (renamedEquippedDynamic)
    {
        this->setSize(0x56);

        // Carefully chosen bits so FUNC_Packet_Incoming_0x000E parses look + longer name (Fellow-style).
        ref<uint8>(0x0A) = 0x57;
        ref<uint8>(0x18) = 0x01;
