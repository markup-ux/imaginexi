...
namespace
{
    std::unordered_map<uint16, std::string> s_traitIdToDisplayName;
}
...
void LoadTraitsList()
{
    s_traitIdToDisplayName.clear();

    auto rset = db::preparedStmt("SELECT traitid, name, job, level, rank, modifier, value, content_tag, meritid "
                                 "FROM traits "
                                 "WHERE traitid < ? "
    ...
    FOR_DB_MULTIPLE_RESULTS(rset)
    {
        ...
        std::string traitName = rset->get<std::string>("name");
        PTrait->setName(traitName);
        s_traitIdToDisplayName[PTrait->getID()] = std::move(traitName);
    ...
    FOR_DB_MULTIPLE_RESULTS(rset)
    {
        ...
        const uint16 tid = PTrait->getID();
        if (auto it = s_traitIdToDisplayName.find(tid); it != s_traitIdToDisplayName.end())
        {
            PTrait->setName(it->second);
        }
