        ...
        [this, host, port]()
        {
            ...
            httpServer_.Get(
                ...
                [&](const httplib::Request& req, httplib::Response& res)
                {
                    ...
                    {
                        limit = std::clamp<uint32>(std::strtoul(req.get_param_value("limit").c_str(), nullptr, 10), 1U, 100U);
                    }

                    auto rset = db::preparedStmt(
                        "SELECT id, itemid, stack, seller_name, buyer_name, sale, sell_date "
                        "FROM auction_house "
                        "WHERE buyer_name IS NOT NULL AND sale > 0 "
                    ...
                    if (rset && rset->rowsCount())
                    {
                        while (rset->next())
                        {
                            j.push_back({
                                ...
                                { "kind", "buy" },
                                { "item_id", rset->get<uint16>("itemid") },
                                { "stack", rset->get<uint8>("stack") },
                                { "price", rset->get<uint32>("sale") },
                                { "seller", rset->get<std::string>("seller_name") },
                                { "buyer", rset->get<std::string>("buyer_name") },
                                { "timestamp", rset->get<uint32>("sell_date") },
                            });
                        }
                    }

                    auto listingRset = db::preparedStmt(
                        "SELECT id, itemid, stack, seller_name, price, date "
                        "FROM auction_house "
                        "WHERE buyer_name IS NULL AND sale = 0 "
                        "ORDER BY date DESC, id DESC "
                        "LIMIT ?",
                        limit);

                    if (listingRset && listingRset->rowsCount())
                    {
                        while (listingRset->next())
                        {
                            j.push_back({
                                { "id", listingRset->get<uint32>("id") },
                                { "kind", "listing" },
                                { "item_id", listingRset->get<uint16>("itemid") },
                                { "stack", listingRset->get<uint8>("stack") },
                                { "price", listingRset->get<uint32>("price") },
                                { "seller", listingRset->get<std::string>("seller_name") },
                                { "buyer", "" },
                                { "timestamp", listingRset->get<uint32>("date") },
                            });
                        }
                    }
