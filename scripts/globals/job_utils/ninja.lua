...
-- Seconds; matches SAM/MNK long stance QoL on this server.
local longStanceDurationS = 3600
...
xi.job_utils.ninja.useYonin = function(player, target, ability, action)
    ...
    target:addStatusEffect(xi.effect.YONIN, { power = 30, duration = longStanceDurationS, origin = player, tick = 15 })
...
xi.job_utils.ninja.useInnin = function(player, target, ability, action)
    target:delStatusEffect(xi.effect.INNIN)
    target:delStatusEffect(xi.effect.YONIN)
    target:addStatusEffect(xi.effect.INNIN, { power = 30, duration = longStanceDurationS, origin = player, tick = 15, subPower = 20 })
...
xi.job_utils.ninja.useFutae = function(player, target, ability, action)
    target:addStatusEffect(xi.effect.FUTAE, { duration = longStanceDurationS, origin = player })

    return xi.effect.FUTAE
end

xi.job_utils.ninja.useIssekigan = function(player, target, ability, action)
    target:addStatusEffect(xi.effect.ISSEKIGAN, { power = 25, duration = longStanceDurationS, origin = player })
