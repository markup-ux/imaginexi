...
bool CNavMesh::tryBuildUnknownZoneFlatMesh()
{
    // Detour world space matches CNavMesh::ToDetourPos (y/z negated vs FFXI). Flat floor at detour y = 0 (FFXI y = 0).
    constexpr float halfXZ       = 2000.0f;
    constexpr float yBelow       = 20.0f;
    constexpr float cs           = 1.0f;
    constexpr float ch           = 1.0f;
    constexpr int   yQuant       = 20; // bmin[1] + yQuant * ch == 0

    const unsigned short span = static_cast<unsigned short>(2.0f * halfXZ / cs);

    unsigned short verts[] = {
        0, static_cast<unsigned short>(yQuant), 0,
        span, static_cast<unsigned short>(yQuant), 0,
        span, static_cast<unsigned short>(yQuant), span,
        0, static_cast<unsigned short>(yQuant), span,
    };

    constexpr unsigned short kPolyVertNone = 0xFFFF;
    unsigned short         polys[DT_VERTS_PER_POLYGON * 2] = {
        0, 1, 2, 3, kPolyVertNone, kPolyVertNone,
        static_cast<unsigned short>(0x8000 | 0xf),
        static_cast<unsigned short>(0x8000 | 0xf),
        static_cast<unsigned short>(0x8000 | 0xf),
        static_cast<unsigned short>(0x8000 | 0xf),
        0,
    ...
    unsigned short polyFlags[] = { SamplePolyFlags::SAMPLE_POLYFLAGS_ALL };
    unsigned char  polyAreas[] = { 0 };
    ...
    params.cs             = cs;
    params.ch             = ch;
    params.bmin[0]        = -halfXZ;
    params.bmin[1]        = -yBelow;
    params.bmin[2]        = -halfXZ;
    params.bmax[0]        = halfXZ;
    params.bmax[1]        = std::max(1.0f, yBelow);
    params.bmax[2]        = halfXZ;

    unsigned char* navData     = nullptr;
    int              navDataSize = 0;
    if (!dtCreateNavMeshData(&params, &navData, &navDataSize) || !navData || navDataSize <= 0)
    {
        ShowError("CNavMesh::tryBuildUnknownZoneFlatMesh dtCreateNavMeshData failed (zone %u)", m_zoneID);
        return false;
    }
    ...
    {
        dtFree(navData);
        ShowError("CNavMesh::tryBuildUnknownZoneFlatMesh dtAllocNavMesh failed (zone %u)", m_zoneID);
        return false;
    }
    ...
    {
        const auto err = detourStatusString(status);
        ShowError("CNavMesh::tryBuildUnknownZoneFlatMesh init failed (zone %u): %s", m_zoneID, err.c_str());
        dtFreeNavMesh(m_navMesh);
        m_navMesh = nullptr;
        dtFree(navData);
        return false;
    ...
    {
        const auto err = detourStatusString(status);
        ShowError("CNavMesh::tryBuildUnknownZoneFlatMesh m_navMeshQuery.init failed (zone %u): %s", m_zoneID, err.c_str());
        dtFreeNavMesh(m_navMesh);
        m_navMesh = nullptr;
        return false;
    }
...
bool CNavMesh::load(const std::string& filename)
{
        ...
        if (header.magic == NAVMESHSET_MAGIC && header.version == NAVMESHSET_VERSION)
        {
            ...
            if (m_navMesh)
            {
                ...
                if (!dtStatusFailed(status))
                {
                    ...
                    const auto err = detourStatusString(status);
                    ShowError("CNavMesh::load Error loading m_navMeshQuery (%s): %s", filename.c_str(), err.c_str());
                }
                else
                {
                    const auto err = detourStatusString(status);
                    ShowError("CNavMesh::load Could not initialize detour for (%s): %s", filename.c_str(), err.c_str());
                }
            }
    ...
    if (m_zoneID == 0)
    {
        ShowInfo("CNavMesh::load (%s) missing or invalid; using runtime flat navmesh for zone 0 (unknown).", filename.c_str());
        return tryBuildUnknownZoneFlatMesh();
    }
