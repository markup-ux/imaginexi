...
local function getAutomatonPet(player)
...
local function getAddBurdenValue(player, maneuverInfo)
    ...
    local auto       = getAutomatonPet(player)
    ...
    if not compareMod then
        return player:getMP() < auto:getMP() and 15 or 10
    else
        return player:getStat(compareMod) < auto:getStat(compareMod) and 20 or 15
    end
end
...
xi.automaton.onUseManeuver = function(player, target, ability, action)
    ...
    local autoPet      = getAutomatonPet(player)

    if
        overload ~= 0 and
        autoPet and
        (player:getMod(xi.mod.PREVENT_OVERLOAD) > 0 or autoPet:getMod(xi.mod.PREVENT_OVERLOAD) > 0) and
    ...
    if overload ~= 0 then
        ...
        local duration = autoPet and autoPet:getLocalVar('MANEUVER_DURATION') or 60
