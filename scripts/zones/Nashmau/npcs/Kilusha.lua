...
entity.onTrigger = function(player, npc)
    ...
    local meetsGate = xi.einherjar.settings.EINHERJAR_FREE_ENTRY or
        (player:getMainLvl() >= xi.einherjar.settings.EINHERJAR_LEVEL_MIN and toau)

    if not meetsGate then
        player:startEvent(22) -- worthless CS
    elseif player:getCharVar('EinherjarIntro') == 1 then
