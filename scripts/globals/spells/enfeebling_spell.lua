...
local pTable =
{
    ...
    -- Virus applies Disease (virus resistance); not Plague (MP/TP drain over time).
    [xi.magic.spell.VIRUS         ] = { xi.effect.DISEASE,            1, xi.mod.INT,    1,   0, 120, 0, false,   0 },
...
xi.spells.enfeebling.useEnfeeblingSpell = function(caster, target, spell)
    ...
    if target:addStatusEffect(spellEffect, { power = potency, duration = duration, origin = caster, tick = tick, subPower = subpotency, tier = tier }) then
        xi.spells.debuffBattleLog.onDebuffApplied(caster, target, spellEffect, potency, duration)
