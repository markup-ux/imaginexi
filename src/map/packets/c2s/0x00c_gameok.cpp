...
void GP_CLI_COMMAND_GAMEOK::process(MapSession* PSession, CCharEntity* PChar) const
{
    ...
    // respawn any pets from last zone (dual pets: restore even in cities when snapshot exists)
    const bool allowPetMisc = PChar->loc.zone->CanUseMisc(MISC_PET) && !PChar->inMogHouse();
    const bool pendingPet   = PChar->shouldPetPersistThroughZoning();
    if (allowPetMisc || pendingPet)
    {
