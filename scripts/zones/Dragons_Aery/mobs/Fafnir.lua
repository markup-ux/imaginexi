...
mixins =
{
    ...
    require('scripts/mixins/hnm_anti_melt'),
    require('scripts/mixins/healer_pressure'),
}
-----------------------------------
---@type TMobEntity
local entity = {}
...
entity.onMobSpawn = function(mob)
    ...
    -- Optional family-native apex mode for Fafnir only.
    -- Uses Wyrm-family skills (Fafnir/Nidhogg skill lists) to avoid cross-rig animation mismatch.
    if xi.settings.map.FAFNIR_FAMILY_NATIVE_APEX_MODE then
        mob:setLocalVar('[hnmApex]replaceDefault', 0)

        -- FafnirWyrm list
        mob:setLocalVar('[hnmApex]skill1', 951) -- Hurricane Wing
        mob:setLocalVar('[hnmApex]skill2', 952) -- Spike Flail (alliance-targeting variant)
        mob:setLocalVar('[hnmApex]skill3', 953) -- Dragon Breath
        mob:setLocalVar('[hnmApex]skill4', 957) -- Absolute Terror
        mob:setLocalVar('[hnmApex]skill5', 958) -- Horrid Roar I

        -- NidhoggWyrm list (same enemy type family)
        mob:setLocalVar('[hnmApex]skill6', 1039) -- Hurricane Wing (Nidhogg variant)
        mob:setLocalVar('[hnmApex]skill7', 1040) -- Spike Flail (outside-alliance variant)
        mob:setLocalVar('[hnmApex]skill8', 1041) -- Dragon Breath (Nidhogg variant)
        mob:setLocalVar('[hnmApex]skill9', 1046) -- Horrid Roar II

        -- Optional conservative cross-family supplements.
        -- These are chosen to be visually low-risk on a dragon rig:
        -- 724 (self evasion buff), 717 (breath-type poison).
        if xi.settings.map.FAFNIR_SAFE_CROSS_FAMILY_APEX_MODE then
            mob:setLocalVar('[hnmApex]skill10', 724) -- Evasion (self buff)
            mob:setLocalVar('[hnmApex]skill11', 717) -- Venom Breath
            mob:setLocalVar('[hnmApex]skill12', 0)
        else
            mob:setLocalVar('[hnmApex]skill10', 0)
            mob:setLocalVar('[hnmApex]skill11', 0)
            mob:setLocalVar('[hnmApex]skill12', 0)
        end
    else
        mob:setLocalVar('[hnmApex]replaceDefault', 0)
