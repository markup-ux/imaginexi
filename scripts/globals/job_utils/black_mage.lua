...
xi.job_utils.black_mage.useCascade = function(player, target, ability)
    -- Long passive window; refresh before expiry if desired (see abilities.sql recastTime).
    player:addStatusEffect(xi.effect.CASCADE, { power = 1, duration = 480, origin = player })
...
-- action: optional CAction from ability script; used to shorten recast when toggling off.
xi.job_utils.black_mage.useManaWall = function(player, target, ability, action)
    ...
    -- Effect cleared on death / zone / job change (status_effects.sql). Long duration until toggled off.
    player:addStatusEffect(xi.effect.MANA_WALL, { power = 1, duration = 604800, origin = player })
