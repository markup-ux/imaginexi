...
-- Tab-purchasable FoV/GoV support buffs (shared with book menu); used when registering a page.
local function applyBookReraise(player, reraisePower)
    player:delStatusEffectSilent(xi.effect.RERAISE)
    player:addStatusEffect(xi.effect.RERAISE, { power = reraisePower, duration = 7200, origin = player })
end

local function applyBookRefresh(player)
...
local function applyBookProtect(player)
...
local function applyBookShell(player)
    local mLvl  = player:getMainLvl()
...
local function applyRegimePageSupportBuffs(player)
    applyBookRefresh(player)
    applyBookReraise(player, 1)
    applyBookProtect(player)
    applyBookShell(player)
end
...
xi.regime.bookOnEventFinish = function(player, option, regimeType)
    ...
    if opt.act then
        ...
        switch (act) : caseof
        {
            ...
            ['RERAISE_II'] = function()
                applyBookReraise(player, 2)
            end,

            ['RERAISE_III'] = function()
                applyBookReraise(player, 3)
            end,
            ...
            ['REFRESH'] = function()
                applyBookRefresh(player) -- Does indeed get overwriten by regular refresh.
            end,
    ...
    elseif opt.page then
        ...
        if page then
            ...
            applyRegimePageSupportBuffs(player)
        end
    end
end
...
xi.regime.checkRegime = function(player, mob, regimeId, index, regimeType)
    ...
    if regimeType == xi.regime.type.GROUNDS then
        ...
        if player:hasStatusEffect(xi.effect.PROWESS) then
            ...
            reward = reward * (100 + (govClears * 10)) / 100
