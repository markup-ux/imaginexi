...
bool isMagicRangeStaffElementalWeaponskill(uint16 wsid)
{
    switch (wsid)
    {
        case 176: // heavy_swing
        case 177: // rock_crusher
        case 178: // earth_crusher
        case 179: // starburst
        case 180: // sunburst
        case 181: // shell_crusher
        case 182: // full_swing
        case 183: // spirit_taker
        case 184: // retribution
        case 185: // gate_of_tartarus
        case 186: // vidohunir
        case 187: // garland_of_bliss
        case 188: // omniscience
        case 189: // cataclysm
        case 190: // myrkr
        case 191: // shattersoul
        case 240: // tartarus_torpor
            return true;
        default:
            return false;
    }
...
float getMagicRangeStaffWeaponskillMaxDistance()
{
    return settings::get<float>("map.STAFF_MAGIC_WS_RANGE");
}

namespace
{
    // Best combat skill value at `level` for any job with non-zero rank (matches gen_weaponskill_availability_md.py cb / best job).
    uint16 GetBestCombatSkillCapAtLevel(SKILLTYPE skillID, uint8 level)
    {
        uint16 best = 0;
        for (uint8 j = 1; j < MAX_JOBTYPE; ++j)
        {
            const auto job = static_cast<JOBTYPE>(j);
            if (GetSkillRank(skillID, job) != 0)
            {
                best = std::max(best, GetMaxSkill(skillID, job, level));
            }
        }
        return best;
    }

    // Keep in sync with server/tools/gen_weaponskill_availability_md.py suggested_level() for sklvl > best cap @ MAX_LEVEL.
    uint8 CompressedWeaponskillMinLevel(uint16 skillRequirement)
    {
        constexpr uint16 lo = 250;
        constexpr uint16 hi = 357;
        const double     num = static_cast<double>(std::max(skillRequirement, lo) - lo);
        const double     den = static_cast<double>(hi - lo);
        const double     t   = den > 0.0 ? std::clamp(num / den, 0.0, 1.0) : 1.0;
        const int        cs  = static_cast<int>(std::lround(61.0 + t * 14.0));
        return static_cast<uint8>(std::clamp(cs, 61, 75));
    }

    bool IsHnmStunProtectedTarget(CBattleEntity* PTarget)
    {
        ...
        if (PMob->m_Pool == 1154) // Dynamis Lord exempt
        {
            return false;
        }

        return PMob->StatusEffectContainer && PMob->StatusEffectContainer->HasStatusEffect(EFFECT_STUN);
    }

    int32 ApplyJobCircleDamageDealtBonus(CBattleEntity* PAttacker, CBattleEntity* PDefender, int32 damage)
    {
        (void)PAttacker;
        (void)PDefender;
        return damage;
    ...
    int32 ApplyJobCircleDamageTakenReduction(CBattleEntity* PDefender, CBattleEntity* PAttacker, int32 damage)
    {
        (void)PDefender;
        (void)PAttacker;
        return damage;
    ...
    int16 GetWsReactionTpBonus(CBattleEntity* PDefender)
    {
        if (!PDefender || PDefender->objtype != TYPE_MOB)
        {
            return 0;
        }

        static constexpr const char* cooldownVar   = "WS_REACTION_CD";
        static constexpr const char* pendingFastCast = "WS_REACTION_FC";

        const uint32 now = earth_time::timestamp();
        if (PDefender->GetLocalVar(cooldownVar) > now)
        {
            return 0;
        }

        const uint32 cooldownSec = settings::get<uint32>("map.WS_REACTION_COOLDOWN_SECONDS");
        PDefender->SetLocalVar(cooldownVar, now + cooldownSec);
        PDefender->SetLocalVar(pendingFastCast, 1);

        auto* PMob = static_cast<CMobEntity*>(PDefender);
        if (PMob->m_Type & MOBTYPE_NOTORIOUS)
        {
            return static_cast<int16>(settings::get<uint16>("map.WS_REACTION_TP_HNM"));
        }

        if ((PMob->m_Type & MOBTYPE_BATTLEFIELD) || PMob->getMobMod(MOBMOD_CHECK_AS_NM) > 0)
        {
            return static_cast<int16>(settings::get<uint16>("map.WS_REACTION_TP_NM"));
        }

        return static_cast<int16>(settings::get<uint16>("map.WS_REACTION_TP_NORMAL"));
    }

    void ApplyWsReactionFastCast(CBattleEntity* PDefender)
    {
        static constexpr const char* pendingFastCast = "WS_REACTION_FC";

        if (!PDefender || !PDefender->StatusEffectContainer || PDefender->GetLocalVar(pendingFastCast) == 0)
        {
            return;
        }

        PDefender->SetLocalVar(pendingFastCast, 0);

        const int16  step        = settings::get<int16>("map.WS_REACTION_FASTCAST_STEP");
        const int16  maxFastCast = settings::get<int16>("map.WS_REACTION_FASTCAST_MAX");
        const auto   duration    = std::chrono::seconds(settings::get<uint16>("map.WS_REACTION_FASTCAST_DURATION_SECONDS"));

        int16 newPower = step;
        if (auto* existing = PDefender->StatusEffectContainer->GetStatusEffect(EFFECT_FAST_CAST))
        {
            newPower = static_cast<int16>(std::min<int32>(existing->GetPower() + step, maxFastCast));
            PDefender->StatusEffectContainer->DelStatusEffect(EFFECT_FAST_CAST);
        }

        PDefender->StatusEffectContainer->AddStatusEffect(
            new CStatusEffect(EFFECT_FAST_CAST, EFFECT_FAST_CAST, newPower, 0s, duration),
            EffectNotice::Silent);
    }
} // namespace
...
{
    // Job / main-only gates disabled (all jobs may use any weaponskill).
    // Skill floor and mythic-style unlock_id still apply. Relic / item-granted WS keep skilllevel 0 + unlock_id 0 in SQL
    // and are added only via Mod::ADDS_WEAPONSKILL in charutils::BuildingCharWeaponSkills (not through this predicate).

    if (PSkill->getSkillLevel() > 0)
    {
        const SKILLTYPE skillType = static_cast<SKILLTYPE>(PSkill->getType());
        const uint16  required    = PSkill->getSkillLevel();

        const bool unlockOk = PSkill->getUnlockId() == 0 || charutils::hasLearnedWeaponskill(PChar, PSkill->getUnlockId());
        if (!unlockOk)
        {
            return false;
        }

        if (PChar->GetSkill(skillType) >= required)
        {
            return true;
        }

        const uint8  serverMax    = settings::get<uint8>("main.MAX_LEVEL");
        const uint16 bestCapAtMax = GetBestCombatSkillCapAtLevel(skillType, serverMax);
        if (required <= bestCapAtMax)
        {
            return false;
        }

        const uint16 maxMainSkill = GetMaxSkill(skillType, PChar->GetMJob(), PChar->GetMLevel());
        const uint16 maxSubSkill  = GetMaxSkill(skillType, PChar->GetSJob(), PChar->GetSLevel());
        uint16       personalCap  = 0;
        if (maxMainSkill != 0)
        {
            personalCap = maxMainSkill;
        }
        else if (maxSubSkill != 0)
        {
            personalCap = maxSubSkill;
        }
        if (personalCap == 0 || PChar->GetSkill(skillType) < personalCap)
        {
            return false;
        }

        return PChar->GetMLevel() >= CompressedWeaponskillMinLevel(required);
    }

    // skillLevel == 0 (mythic / empyrean / aeonic style): require explicit learned flag and server max job level.
    if (PSkill->getUnlockId() == 0)
    {
        return false;
    }

    const uint8 serverMax = settings::get<uint8>("main.MAX_LEVEL");
    return charutils::hasLearnedWeaponskill(PChar, PSkill->getUnlockId()) && PChar->GetMLevel() >= serverMax;
}
...
int32 CalculateEnspellDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, uint8 Tier, uint8 element, CItemWeapon* pWeaponHit)
{
    ...
    damage = ApplyJobCircleDamageDealtBonus(PAttacker, PDefender, damage);
    damage = MagicDmgTaken(PDefender, damage, (ELEMENT)(element));
    damage = ApplyJobCircleDamageTakenReduction(PDefender, PAttacker, damage);
...
{
    if (settings::get<bool>("map.WS_REACTION_ENABLED"))
    {
        const int16 tpBonus = GetWsReactionTpBonus(PDefender);
        if (tpBonus > 0)
        {
            PDefender->addTP(tpBonus);
        }

        ApplyWsReactionFastCast(PDefender);
    }

    if (IsHnmStunProtectedTarget(PDefender))
    {
        if (primary && PAttacker && PAttacker->objtype == TYPE_PC)
        {
            if (auto* PWsState = dynamic_cast<CWeaponSkillState*>(PAttacker->PAI->GetCurrentState()))
            {
                const int16 spentTP = PWsState->GetSpentTP();
                if (spentTP > 0)
                {
                    PAttacker->addTP(spentTP);
                }
            }
        }
...
namespace
{
    bool IsMagicTpEnabled()
    {
        return settings::get<bool>("map.MAGIC_TP_ENABLED");
    }

    bool IsMagicTpEligibleJob(CBattleEntity* caster)
    {
        if (caster == nullptr || caster->objtype != TYPE_PC)
        {
            return false;
        }

        switch (caster->GetMJob())
        {
            case JOB_BLM:
            case JOB_WHM:
            case JOB_RDM:
    ...
    void LogMagicTpGain(CBattleEntity* caster, int16 tpDelta, const char* source, const char* detail)
    {
        if (!settings::get<bool>("map.MAGIC_TP_LOG_ENABLED") || caster == nullptr || tpDelta == 0)
        {
            return;
        }
        ...
        {
            ShowInfo("[{}] TP {:+d} ({} -> {}) [{}] {}", caster->getName(), tpDelta, caster->health.tp - tpDelta, caster->health.tp, source, detail);
        }
        else
        {
            ShowInfo("[{}] TP {:+d} ({} -> {}) [{}]", caster->getName(), tpDelta, caster->health.tp - tpDelta, caster->health.tp, source);
        }
    }

    int16 CalculateMeleeEquivalentCastTp(CBattleEntity* caster)
    {
        if (!IsMagicTpEnabled() || !IsMagicTpEligibleJob(caster))
        {
            return 0;
        }

        auto* subWeapon  = dynamic_cast<CItemWeapon*>(caster->m_Weapons[SLOT_SUB]);
        auto* mainWeapon = dynamic_cast<CItemWeapon*>(caster->m_Weapons[SLOT_MAIN]);

        if (settings::get<bool>("map.MAGIC_TP_REQUIRES_STAFF"))
        {
            if (mainWeapon == nullptr || mainWeapon->getSkillType() != SKILL_STAFF)
            {
                return 0;
            }
        }
        ...
        {
            delay = caster->GetWeaponDelay(true);
            if (subWeapon && subWeapon->getDmgType() > DAMAGE_TYPE::NONE && subWeapon->getDmgType() < DAMAGE_TYPE::HTH && mainWeapon->getSkillType() != SKILL_HAND_TO_HAND)
            {
                delay = delay / 2;
            }
        }
        ...
        {
            delay = settings::get<uint16>("map.MAGIC_TP_SYNTHETIC_DELAY");
        }

        float ratio = 1.0f;
        if (mainWeapon && mainWeapon->getSkillType() == SKILL_HAND_TO_HAND)
        {
            ratio = 2.0f;
        }

        int16 const baseTp = CalculateBaseTP(caster, static_cast<int32>(delay * 60.0f / 1000.0f / ratio));
        int16 const castTp = static_cast<int16>(
            baseTp * (1.0f + 0.01f * static_cast<float>(caster->getMod(Mod::STORETP) + getStoreTPbonusFromMerit(caster))));
        return std::max<int16>(1, castTp);
    }

    int16 ScaleMagicTp(int16 baseTp, float multiplier)
    {
        return static_cast<int16>(std::max<float>(1.0f, static_cast<float>(baseTp) * multiplier));
    }

    void GrantMagicTp(CBattleEntity* caster, int16 tpGain, const char* source, const char* detail = nullptr)
    {
        if (caster == nullptr || tpGain <= 0)
        {
            return;
        }

        int16 const prevTp = caster->health.tp;
        caster->addTP(tpGain);
        LogMagicTpGain(caster, static_cast<int16>(caster->health.tp - prevTp), source, detail);
    }

    void TrackDoTCaster(CBattleEntity* caster, CBattleEntity* target, CSpell* spell)
    {
        if (!IsMagicTpEnabled() || caster == nullptr || target == nullptr || spell == nullptr || caster->objtype != TYPE_PC || !spell->tookEffect())
        {
            return;
        }

        if (!IsMagicTpEligibleJob(caster))
        {
            return;
        }
        ...
        switch (spell->getSpellFamily())
        {
            case SPELLFAMILY_BIO:
                target->SetLocalVar("magicTpBioCasterId", caster->id);
                break;
            case SPELLFAMILY_POISON:
            case SPELLFAMILY_POISONGA:
                target->SetLocalVar("magicTpPoisonCasterId", caster->id);
                break;
            case SPELLFAMILY_ELE_DOT:
                target->SetLocalVar("magicTpElementalDotCasterId", caster->id);
                break;
            ...
            case SPELLFAMILY_LUMINOHELIX:
                target->SetLocalVar("magicTpHelixCasterId", caster->id);
                break;
            default:
                break;
        }
    ...
    void GrantDoTTickTpToCasters(CBattleEntity* defender)
    {
        if (!IsMagicTpEnabled() || defender == nullptr)
        {
            return;
        }

        std::array<const char*, 4> casterVars{
            "magicTpBioCasterId",
            "magicTpPoisonCasterId",
            "magicTpElementalDotCasterId",
            "magicTpHelixCasterId",
        };
        std::array<uint32, 3> uniqueCasterIds{};
        size_t                uniqueCount = 0;

        for (const auto* varName : casterVars)
        {
            auto casterId = static_cast<uint32>(defender->GetLocalVar(varName));
            if (casterId == 0)
            {
                continue;
            }
            ...
            if (!exists && uniqueCount < uniqueCasterIds.size())
            {
                uniqueCasterIds[uniqueCount++] = casterId;
            }
        }

        for (size_t i = 0; i < uniqueCount; ++i)
        {
            if (auto* PEntity = defender->GetEntity(uniqueCasterIds[i]))
            {
                if (auto* PCaster = dynamic_cast<CBattleEntity*>(PEntity); PCaster && PCaster->objtype == TYPE_PC && PCaster->isAlive())
                {
                    int16 const castTp    = CalculateMeleeEquivalentCastTp(PCaster);
                    float const dotMult   = settings::get<float>("map.MAGIC_TP_DOT_MULT");
                    int16 const dotTickTp = ScaleMagicTp(static_cast<int16>(castTp / 2), dotMult);
                    GrantMagicTp(PCaster, dotTickTp, "dot_hp_tick_caster_tp");
                }
            }
        }
    }
...
void OnPlayerSpellCastFinished(CBattleEntity* caster, CBattleEntity* target, CSpell* spell, int32 spellResult)
{
    if (!IsMagicTpEnabled() || caster == nullptr || target == nullptr || spell == nullptr || caster->objtype != TYPE_PC)
    {
        return;
    }

    TrackDoTCaster(caster, target, spell);

    if (spell->isDebuff() && spell->tookEffect())
    {
        int16 const tpGain = ScaleMagicTp(CalculateMeleeEquivalentCastTp(caster), settings::get<float>("map.MAGIC_TP_ENFEEBLE_MULT"));
        GrantMagicTp(caster, tpGain, "spell_enfeeble_land");
        return;
    }

    if (spell->isHeal() && spellResult > 0 && target->GetMaxHP() > 0)
    {
        float const healRatio = std::min(1.0f, static_cast<float>(spellResult) / static_cast<float>(target->GetMaxHP()));
        int16 const tpGain    = ScaleMagicTp(CalculateMeleeEquivalentCastTp(caster), settings::get<float>("map.MAGIC_TP_HEAL_MULT") * healRatio);
        GrantMagicTp(caster, tpGain, "spell_heal_tp");
    }
}

void OnPlayerDoTPoisonTick(CBattleEntity* defender)
{
    GrantDoTTickTpToCasters(defender);
}

void OnPlayerTpChanged(CBattleEntity* player, uint16 prevTp)
{
    if (player == nullptr || player->objtype != TYPE_PC)
    {
        return;
    }

    auto onPlayerTpChanged = lua["xi"]["magicTp"]["onPlayerTpChanged"];
    if (!onPlayerTpChanged.valid())
    {
        return;
    }

    auto result = onPlayerTpChanged(CLuaBaseEntity(player), static_cast<int32>(prevTp));
    if (!result.valid())
    {
        sol::error err = result;
        ShowError("battleutils::OnPlayerTpChanged: %s", err.what());
    }
}

namespace
{
    void GrantPlayerMagicNukeTpFromAppliedDamage(CBattleEntity* attacker, CSpell* spell, int32 appliedDamage)
    {
        if (!IsMagicTpEnabled() || attacker == nullptr || spell == nullptr || attacker->objtype != TYPE_PC || !spell->canTargetEnemy() || appliedDamage <= 0)
        {
            return;
        }
        ...
        int16 const tpGain = ScaleMagicTp(CalculateMeleeEquivalentCastTp(attacker), settings::get<float>("map.MAGIC_TP_NUKE_MULT"));
        char        detail[160]{};
        auto const  spellId = static_cast<uint16>(spell->getID());
        snprintf(detail, sizeof(detail), "spell_id=%u spell=%s dmg=%d melee_equiv=1to1", spellId, spell->getName().c_str(), appliedDamage);
        GrantMagicTp(attacker, tpGain, "spell_nuke_damage_tp", detail);
    }
} // namespace
...
int32 TakeSpellDamage(CBattleEntity* PDefender, CBattleEntity* PAttacker, CSpell* PSpell, int32 damage, ATTACK_TYPE attackType, DAMAGE_TYPE damageType)
{
    if (IsHnmStunProtectedTarget(PDefender))
    {
        ...
        {
            if (auto* PMagicState = dynamic_cast<CMagicState*>(PAttacker->PAI->GetCurrentState()))
            {
                CSpell* PCurrentSpell = PMagicState->GetSpell();
                if (PCurrentSpell && PCurrentSpell->hasMPCost() && !PAttacker->StatusEffectContainer->HasStatusEffect(EFFECT_MANAFONT))
                {
                    const int16 refund = battleutils::CalculateSpellCost(PAttacker, PCurrentSpell);
                    if (refund > 0)
                    {
                        PAttacker->addMP(refund);
                    }
                }
            }
        }
    ...
    int32 const spellCorrected = PDefender->takeDamage(damage, PAttacker, attackType, damageType);

    // Remove effects from damage
    if (PSpell->canTargetEnemy() && spellCorrected > 0)
    {
        PDefender->StatusEffectContainer->DelStatusEffectsByFlag(EFFECTFLAG_DAMAGE);
        ...
        // Add TP for damaging spells (Occult Acumen trait)
        ...
        GrantPlayerMagicNukeTpFromAppliedDamage(PAttacker, PSpell, spellCorrected);
    }
...
namespace
{
    const char* SkillchainPropertyDisplayName(SKILLCHAIN_ELEMENT sc)
    {
        switch (sc)
        {
            case SC_NONE:
    ...
    const char* ElementDisplayName(ELEMENT e)
    {
        switch (e)
        {
            case ELEMENT_FIRE:
    ...
    void BroadcastSkillchainDetail(CBattleEntity* PAttacker, CBattleEntity* PDefender, SKILLCHAIN_ELEMENT sc, uint8_t propertyTier)
    {
        if (PDefender == nullptr || PDefender->loc.zone == nullptr || sc == SC_NONE || propertyTier == 0)
        {
            return;
        }

        const auto               elems = GetSkillchainMagicElement(sc);
        std::vector<std::string> elemParts;
        elemParts.reserve(elems.size());
        for (ELEMENT ele : elems)
        {
            elemParts.emplace_back(ElementDisplayName(ele));
        }

        std::string elemStr;
        for (std::size_t i = 0; i < elemParts.size(); ++i)
        {
            if (i > 0)
            {
                elemStr += ", ";
        ...
        const std::string msg = fmt::format("Skillchain: {} | {} | Lv. {}",
                                            SkillchainPropertyDisplayName(sc),
                                            elemStr,
                                            static_cast<int>(propertyTier));

        PDefender->loc.zone->PushPacket(PDefender, CHAR_INRANGE,
                                        std::make_unique<GP_SERV_COMMAND_CHAT_STD>("", PDefender->getZone(), MESSAGE_SYSTEM_1, msg, 0));
    }
} // namespace
...
int32 TakeSkillchainDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, int32 lastSkillDamage, CBattleEntity* taChar)
{
    ...
    BroadcastSkillchainDetail(PAttacker, PDefender, skillchain, static_cast<uint8_t>(chainLevel));
...
uint8 getBarrageShotCount(CCharEntity* PChar)
{
    ...
    */
    ...
    if (!settings::get<bool>("map.DISABLE_AMMO_CONSUMPTION") && PAmmo && PAmmo->getQuantity() < shotCount)
    {
...
void applyCharm(CBattleEntity* PCharmer, CBattleEntity* PVictim, timer::duration charmTime)
{
    ...
    if (PVictim->objtype == TYPE_MOB)
    {
        ...
        if (PCharmer->PPet != nullptr)
        {
            PVictim->isCharmed = false;
            PVictim->PMaster   = nullptr;
            return;
        }
        PCharmer->PPet = PMob;
        ...
        PMob->PAI->SetController(std::make_unique<CPetController>(PMob));
        if (PChar && PChar->GetMJob() == JOB_BST)
        {
            PMob->charmTime = timer::time_point::max();
        }
        else
        {
            PMob->charmTime = timer::now() + charmTime;
        }
        ...
        if (PChar)
        {
            ...
            charutils::RebuildCharPetAbilityTable(PChar);
    ...
    {
        if (PVictim->HasAnyPet())
        {
            const auto pets = PVictim->GetAllPets();
            for (CBattleEntity* pet : pets)
            {
                if (pet != nullptr)
                {
                    petutils::DespawnPet(PVictim, pet);
                }
            }
        }
...
{
    if (settings::get<bool>("map.DISABLE_AMMO_CONSUMPTION"))
    {
        return false;
    }
