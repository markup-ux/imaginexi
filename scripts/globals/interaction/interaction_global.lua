...
function InteractionGlobal.loadContainers(shouldReloadRequires)
    ...
    for i = 1, #containerFiles do
        local container = utils.prequire(containerFiles[i])
        if container then
            container.filename = containerFiles[i]
            containers[#containers + 1] = container
        else
            printf('InteractionGlobal.loadContainers: failed to load %s', containerFiles[i])
        end
    end
