...
void CParty::AddMember(CBattleEntity* PEntity)
{
    ...
    if (m_PartyType == PARTY_PCS)
    {
        ...
        if (m_PSyncTarget != nullptr)
        {
            ...
            {
                auto*          PSyncTarget  = static_cast<CCharEntity*>(m_PSyncTarget);
                const uint8    syncPow    = std::min<uint8>(PSyncTarget->jobs.job[PSyncTarget->GetMJob()], settings::get<uint8>("main.MAX_LEVEL"));
                PChar->pushPacket<GP_SERV_COMMAND_BATTLE_MESSAGE>(PChar, PChar, 0, syncPow, MsgStd::LevelSyncActivated);
    ...
    if (settings::get<bool>("map.LEVEL_SYNC_ENABLE"))
    {
        if (PEntity && PEntity->objtype == TYPE_PC)
        {
            ...
            else
            {
                ...
                m_PSyncTarget = PChar;
                const uint8 syncPartyLevel = std::min<uint8>(PChar->jobs.job[PChar->GetMJob()], settings::get<uint8>("main.MAX_LEVEL"));
                for (auto& i : members)
                {
                    ...
                    {
                        member->pushPacket<GP_SERV_COMMAND_MESSAGE>(syncPartyLevel, 0, 0, 0, message);
                        member->StatusEffectContainer->DelStatusEffectsByFlag(EFFECTFLAG_DISPELABLE | EFFECTFLAG_ON_ZONE);
                        member->StatusEffectContainer->AddStatusEffect(new CStatusEffect(EFFECT_LEVEL_SYNC, EFFECT_LEVEL_SYNC, syncPartyLevel, 0s, 0s), EffectNotice::Silent);
...
void CParty::RefreshSync()
{
    ...
    uint8        syncLevel = std::min<uint8>(sync->jobs.job[sync->GetMJob()], settings::get<uint8>("main.MAX_LEVEL"));
    if (syncLevel < 10)
    {
    ...
    for (auto& i : members)
    {
        ...
        // True sync: everyone matches the designee's level (bidirectional — members below sync up).
        uint8 NewMLevel = std::max<uint8>(1, syncLevel);
        ...
        if (member->GetMLevel() != NewMLevel)
        {
            ...
            charutils::BuildingCharTraitsTable(member, true);
            charutils::BuildingCharAbilityTable(member, true);
