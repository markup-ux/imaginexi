...
xi.job_utils.dark_knight.useArcaneCircle = function(player, target, ability)
    -- Killer: power (+ potency). Affinity vs. arcana: subPower (battleutils).
    -- https://www.bg-wiki.com/ffxi/Arcane_Circle
    local longCircleDurationS = 3600
    local duration            = longCircleDurationS + player:getMod(xi.mod.ARCANE_CIRCLE_DURATION)
    local power               = 15
    local subPower            = 15
    ...
    if player:getMainJob() ~= xi.job.DRK then
        ...
        subPower = subPower + player:getJobPointLevel(xi.jp.ARCANE_CIRCLE_EFFECT)
    end

    power = power + player:getMod(xi.mod.ARCANE_CIRCLE_POTENCY)

    target:addStatusEffect(xi.effect.ARCANE_CIRCLE, { power = power, subPower = subPower, duration = duration, origin = player })
