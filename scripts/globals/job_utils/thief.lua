...
xi.job_utils.thief.useLarceny = function(player, target, ability, action)
    ...
    if effectStolen == nil then
        ...
        if newStatus then
            ...
            newStatus:addEffectFlag(xi.effectFlag.ON_ZONE)
        end
    -- Copy an SP Ability if found
        ...
        player:addStatusEffect(newID, { power = newPower, duration = newDuration, origin = player, tick = newTick, icon = newIcon, subType = newSubType, subPower = newSubPower, tier = newTier, flag = bit.bor(newFlags, xi.effectFlag.ON_ZONE) })
...
-----------------------------------
-- Crit recast reduction (main THF): auto-attack crits shave time off all job-ability
-- recasts except one-hour / Lv96 two-hour / mount (see CRecastContainer::ReduceRecasts).
-----------------------------------
local thfCritRecastListenerId = 'THF_CRIT_REDUCE_JA_RECAST'

-- Recommended starting value: 1 second per crit — meaningful over a long fight without
-- behaving like a second Haste for short-recast abilities. Increase for a stronger feel.
local critRecastReductionSeconds = 1

local function onThfMeleeSwingCrit(player, target, attack)
    if not attack:isCritical() then
        return
    end

    player:reduceAllAbilityRecasts(critRecastReductionSeconds)
end

xi.job_utils.thief.updateCritRecastReductionListener = function(player)
    player:removeListener(thfCritRecastListenerId)

    if player:getMainJob() == xi.job.THF then
        player:addListener('MELEE_SWING_HIT', thfCritRecastListenerId, onThfMeleeSwingCrit)
    end
end
