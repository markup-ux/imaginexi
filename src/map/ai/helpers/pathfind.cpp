...
bool CPathFind::RoamAround(const position_t& point, float maxRadius, uint8 maxTurns, uint16 roamFlags)
{
    ...
    else
    {
        ...
        // Zone 0 (UNKNOWN): no retail navmesh; the legacy fallback is a 1 yalm jitter so mobs never roam.
        if (m_POwner->loc.zone && m_POwner->loc.zone->GetID() == 0)
        {
            const float radius = maxRadius < 2.0f ? 2.0f : maxRadius;
            position_t dest{};
            dest.x        = point.x + xirand::GetRandomNumber<float>(-radius, radius);
            dest.y        = point.y;
            dest.z        = point.z + xirand::GetRandomNumber<float>(-radius, radius);
            dest.moving   = 0;
            dest.rotation = 0;
            m_points.emplace_back(pathpoint_t{ dest, 0s, false });
        }
        else
        {
