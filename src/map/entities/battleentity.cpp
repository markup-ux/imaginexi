...
namespace
{
bool isDynamisLordMob(const CBattleEntity* entity)
{
    if (!entity || entity->objtype != TYPE_MOB)
    {
        return false;
    }

    const auto* mob = static_cast<const CMobEntity*>(entity);
    return std::string_view{ mob->getName() } == "Dynamis Lord";
}

bool isHnmStunProtectionActive(const CBattleEntity* entity)
{
    if (!entity || entity->objtype != TYPE_MOB)
    {
        return false;
    ...
    const auto* mob = static_cast<const CMobEntity*>(entity);
    return (mob->m_Type & MOBTYPE_NOTORIOUS) && !isDynamisLordMob(entity) && entity->StatusEffectContainer->HasStatusEffect(EFFECT_STUN);
}
...
std::vector<CBattleEntity*> CBattleEntity::GetAllPets() const
{
...
bool CBattleEntity::HasAnyPet() const
{
    return PPet != nullptr;
}

void CBattleEntity::AttachPet(CBattleEntity* pet)
{
    if (pet == nullptr)
    {
        return;
...
bool CBattleEntity::RemovePetPointer(CBattleEntity* pet)
{
    if (pet == nullptr)
    {
        return false;
...
{
    if (mp < 0 && isHnmStunProtectionActive(this))
    {
        return 0;
    }
...
int32 CBattleEntity::takeDamage(int32 amount, CBattleEntity* attacker /* = nullptr*/, ATTACK_TYPE attackType /* = ATTACK_NONE*/, DAMAGE_TYPE damageType /* = DAMAGE_NONE*/, bool isSkillchainDamage /* = false */)
{
    ...
    if (amount > 0 && isHnmStunProtectionActive(this))
    {
        return 0;
    }
...
void CBattleEntity::OnCastFinished(CMagicState& state, action_t& action)
{
    ...
    for (auto* PTarget : PAI->TargetFind->m_targets)
    {
        ...
        else
        {
            ...
            {
                battleutils::OnPlayerSpellCastFinished(this, PTarget, PSpell, damage);
            }
        }
