...
require('scripts/globals/pixie_rescue')
require('scripts/globals/quests')
require('scripts/globals/teleports')
require('scripts/globals/unlock_all_gated_access')
require('scripts/globals/race_tied_quest_reset')
...
local startingJobGear =
{
    ...
    [xi.job.WHM] = { xi.item.ONION_ROD,   },
    [xi.job.BLM] = { xi.item.ONION_STAFF, },
    [xi.job.RDM] = { xi.item.ONION_DAGGER,},
    [xi.job.THF] = { xi.item.ONION_KNIFE                               },
}

local maxPlayerStorageSize = 80
local maxPlayerStorageContainers =
{
    xi.inv.INVENTORY,
    xi.inv.MOGSAFE,
    xi.inv.STORAGE,
...
local function ensureMaxStorageForAllPlayers(player)
    for _, containerId in ipairs(maxPlayerStorageContainers) do
        local currentSize = player:getContainerSize(containerId)
        if currentSize < maxPlayerStorageSize then
            player:changeContainerSize(containerId, maxPlayerStorageSize - currentSize)
        end
    end
end

-- Adoulin jobs: unlock for everyone (char create only covered PLD–SCH; existing chars need login hook).
local function ensureGeoRunUnlocked(player)
    if not player:hasJob(xi.job.GEO) then
        player:unlockJob(xi.job.GEO)
    end

    if not player:hasJob(xi.job.RUN) then
        player:unlockJob(xi.job.RUN)
    end
end

local mnkKickAttackRateTarget = 38
local mnkKickAttackRateBonusVar = '[mnkKickAttackRate38]bonus'

local function applyMonkKickAttackRateCap(player)
    local previousBonus = player:getLocalVar(mnkKickAttackRateBonusVar)
    if previousBonus ~= 0 then
        player:delMod(xi.mod.KICK_ATTACK_RATE, previousBonus)
        player:setLocalVar(mnkKickAttackRateBonusVar, 0)
    end
    ...
    local currentRate = player:getMod(xi.mod.KICK_ATTACK_RATE)
    local neededBonus = mnkKickAttackRateTarget - currentRate

    if neededBonus ~= 0 then
        player:addMod(xi.mod.KICK_ATTACK_RATE, neededBonus)
        player:setLocalVar(mnkKickAttackRateBonusVar, neededBonus)
    end
end

local weeklyBeastmanTribes =
{
    'Orcs',
    'Quadav',
    'Yagudo',
...
local function getWeeklyBeastmanTribe()
    local weekIndex = math.floor(os.time(os.date('!*t')) / (60 * 60 * 24 * 7))
    local tribeIdx  = (weekIndex % #weeklyBeastmanTribes) + 1
    return weeklyBeastmanTribes[tribeIdx]
end
...
xi.player.charCreate = function(player)
    ...
    ensureGeoRunUnlocked(player)
    ensureMaxStorageForAllPlayers(player)
end

-- called by core after char_look race/face/size is updated (before rezone). PChar look is still pre-change until zone reload.
xi.player.onRaceChange = function(player)
    xi.raceTiedQuestReset.onRaceChange(player)
end

-- called by core after a player logs into the server or zones
xi.player.onGameIn = function(player, firstLogin, zoning)
    -- charCreate must run on every first login. When char_stats.zoning is 1 (e.g. disconnect
    -- mid-zone), the old `if not zoning` guard skipped charCreate entirely (0 gil, missing titles).
    if firstLogin then
        xi.player.charCreate(player)
    end

    if not zoning then
        local weeklyTribe = getWeeklyBeastmanTribe()
        player:printToPlayer(string.format('Weekly Bonus: %s beastmen grant +50%% Conquest Points and Imperial Standing.', weeklyTribe), xi.msg.channel.SYSTEM_3)
    else
    ...
    xi.unlock_all_gated_access.grantAccessUnlocks(player)

    ensureGeoRunUnlocked(player)
    ensureMaxStorageForAllPlayers(player)
    applyMonkKickAttackRateCap(player)
    xi.job_utils.thief.updateCritRecastReductionListener(player)
...
xi.player.onPlayerRaiseAccept = function(player)
    xi.pixieRescue.onPlayerRaiseAccept(player)
end
...
-- Called from core after mog menu / my room job change is applied (see c2s 0x100).
xi.player.onMoghouseJobChange = function(player)
    xi.job_utils.thief.updateCritRecastReductionListener(player)
end
