...
namespace
{
    CParty* partyOfCaster(CBattleEntity* PInitiator)
    {
        if (PInitiator->PParty)
        {
            return PInitiator->PParty;
        }
        if (PInitiator->PMaster && PInitiator->PMaster->objtype == TYPE_PC)
        {
            return static_cast<CCharEntity*>(PInitiator->PMaster)->PParty;
        }
        return nullptr;
    }
} // namespace
...
void CPetEntity::PostTick()
{
    ...
    if (loc.zone && updatemask && status != STATUS_TYPE::DISAPPEAR && now > m_nextUpdateTimer)
    {
        ...
        if (PMaster && PMaster->objtype == TYPE_PC && PMaster->PPet == this)
        {
...
void CPetEntity::Die()
{
    ...
    if (PMaster && PMaster->objtype == TYPE_PC && PMaster->PPet == this)
    {
        petutils::DetachPet(PMaster, this);
    }
}
...
void CPetEntity::loadPetZoningInfo()
{
    ...
    {
        const PetInfo_t& info = master->petZoningInfo;

        health.tp = static_cast<uint16>(info.petTP);
        health.hp = info.petHP;
        health.mp = info.petMP;

        if (m_PetType == PET_TYPE::JUG_PET)
        {
            setJugDuration(info.jugDuration);
            setJugSpawnTime(info.jugSpawnTime);
        }
    }
}
...
bool CPetEntity::ValidTarget(CBattleEntity* PInitiator, uint16 targetFlags)
{
    ...
    // Party / alliance buffs & heals whose validTargets omit TARGET_PLAYER (e.g. Regen: SELF|PARTY).
    auto* PMasterPC = dynamic_cast<CCharEntity*>(PMaster);
    if (!PMasterPC)
    {
        return false;
    }

    CParty* PInitParty       = partyOfCaster(PInitiator);
    bool    isSameParty      = PMasterPC->PParty && PInitParty && PInitParty == PMasterPC->PParty;
    bool    isSameAlliance   = PMasterPC->PParty && PMasterPC->PParty->m_PAlliance && PInitParty && PInitParty->m_PAlliance &&
                            PMasterPC->PParty->m_PAlliance == PInitParty->m_PAlliance;
    bool    isPartyPetMaster = PInitiator->PMaster && PInitiator->PMaster->PParty && PInitiator->PMaster->PParty == PMasterPC->PParty;
    bool    isSoloPetMaster  = PMasterPC->PParty == nullptr && PInitiator->PMaster == PMasterPC;
