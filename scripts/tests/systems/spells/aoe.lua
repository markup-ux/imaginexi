describe('AoE', function()
    ...
    describe('Accession #sch', function()
        ...
        it('allows multiple RADIAL_ACCE spells while Accession is active', function()
            p1.actions:useSpell(p1, xi.magic.spell.PROTECT)
            xi.test.world:tickEntity(p1)
            xi.test.world:skipTime(5)
            p1.assert:hasEffect(xi.effect.ACCESSION)
            ...
            xi.test.world:skipTime(5)
            p1.assert:hasEffect(xi.effect.SHELL)
            p2.assert:hasEffect(xi.effect.SHELL)
            p1.assert:hasEffect(xi.effect.ACCESSION)
        end)
    ...
    describe('Manifestation #sch', function()
        ...
        it('converts RADIAL_MANI spells to 10y AoE', function()
            p1:addStatusEffect(xi.effect.MANIFESTATION, { power = 1, duration = 45, origin = p1 })
        ...
        it('still converts a second RADIAL_MANI spell while Manifestation is active', function()
            p1:addStatusEffect(xi.effect.MANIFESTATION, { power = 1, duration = 45, origin = p1 })
            local sleepSpell = GetSpell(xi.magic.spell.SLEEP)
            local bindSpell  = GetSpell(xi.magic.spell.BIND)
            assert(sleepSpell, 'SLEEP spell not found')
            assert(bindSpell, 'BIND spell not found')
            local r1 = xi.combat.magicAoE.calculateTypeAndRadius(p1, sleepSpell)
            assert(r1[1] == xi.magic.aoe.RADIAL and r1[2] == 10, 'First RADIAL_MANI spell should be 10y AoE')
            local r2 = xi.combat.magicAoE.calculateTypeAndRadius(p1, bindSpell)
            assert(r2[1] == xi.magic.aoe.RADIAL and r2[2] == 10, 'Second RADIAL_MANI spell should still be 10y AoE')
            p1.assert:hasEffect(xi.effect.MANIFESTATION)
        end)
