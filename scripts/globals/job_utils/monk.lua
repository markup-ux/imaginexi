...
-- Seconds; matches SAM Hasso/Seigan stance QoL on this server.
local longStanceDurationS = 3600
local boostDurationS      = longStanceDurationS
...
xi.job_utils.monk.useBoost = function(player, target, ability)
    ...
    if player:hasStatusEffect(xi.effect.BOOST) then
        ...
        effect:setDuration(boostDurationS * 1000) -- ms; refresh timer when stacking
        effect:resetStartTime()
    else
        player:addStatusEffect(xi.effect.BOOST, { power = power, duration = boostDurationS, origin = player })
    end
end
...
xi.job_utils.monk.useHundredFists = function(player, target, ability)
    ...
    if player:isEngaged() then
        local targetId = player:getTargID()
        if targetId and targetId > 0 then
            player:timer(50, function(p)
                if p and p:isEngaged() then
                    p:engage(targetId)
                end
            end)
        end
