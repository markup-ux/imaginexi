...
xi.job_utils.samurai.useWardingCircle = function(player, target, ability)
    ...
    local duration            = longCircleDurationS + player:getMod(xi.mod.WARDING_CIRCLE_DURATION)
    local power               = 15
    local subPower            = 15
    ...
    if player:getMainJob() ~= xi.job.SAM then
        ...
        subPower = subPower + player:getJobPointLevel(xi.jp.WARDING_CIRCLE_EFFECT)
    end

    power = power + player:getMod(xi.mod.WARDING_CIRCLE_POTENCY)

    target:addStatusEffect(xi.effect.WARDING_CIRCLE, { power = power, subPower = subPower, duration = duration, origin = player })
