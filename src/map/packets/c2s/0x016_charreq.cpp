...
void GP_CLI_COMMAND_CHARREQ::process(MapSession* PSession, CCharEntity* PChar) const
{
    ...
    // Use a broad filter: client slot can be NPC, ship transport, or (rarely) mob in the low targid range.
    CBaseEntity* PEntity = PChar->GetEntity(this->ActIndex, TYPE_NPC | TYPE_PC | TYPE_MOB | TYPE_SHIP);
    if (!PEntity)
    {
    ...
    else // NPC, ship, mob, etc.
    {
        // Special case for onZoneIn cutscenes in Mog House (NPCs only)
