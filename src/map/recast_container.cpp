...
/************************************************************************
 *                                                                       *
 *  Reduce remaining time on active recasts (by shifting end time        *
 *  earlier). Charge-based entries use the same end-time model as        *
 *  HasRecast (TimeStamp + RecastTime).                                 *
 *                                                                       *
 ************************************************************************/

void CRecastContainer::ReduceRecasts(RECASTTYPE type, timer::duration amount)
{
    if (amount <= 0s)
    {
        return;
    ...
    for (auto& recast : *PRecastList)
    {
        ...
        if (type == RECAST_ABILITY)
        {
            if (recast.ID == Recast::Special || recast.ID == Recast::Special2 || recast.ID == Recast::Mount)
            {
                continue;
            }
        }

        auto const end    = recast.TimeStamp + recast.RecastTime;
        auto const newEnd = end - amount;
        if (newEnd <= timer::now())
        {
            recast.RecastTime = 0s;
        }
        ...
        {
            recast.TimeStamp = newEnd - recast.RecastTime;
        }
    }
}
