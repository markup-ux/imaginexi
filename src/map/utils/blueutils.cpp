...
{
    // ImagineXI: BLU spells are granted by level progression (GrantLevelEligibleSpells).
    if (!settings::get<bool>("map.BLU_MOB_LEARN_ENABLED"))
    {
        return;
    }
    ...
    for (auto PBlueMage : PBlueMages)
    {
        ...
        for (auto PSpell : PLearnableSpells)
        {
            ...
            if (playerSkillLvl >= skillLvlForSpell - 31)
            {
                ...
                {
                    if (charutils::addSpell(PBlueMage, static_cast<uint16>(PSpell->getID()), true))
                    {
