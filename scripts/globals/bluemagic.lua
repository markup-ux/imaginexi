...
require('scripts/globals/spells/debuff_battle_log')
-----------------------------------
xi = xi or {}
...
xi.spells.blue.usePhysicalSpell = function(caster, target, spell, params)
    ...
    -- Hits that passed the physical accuracy check (excludes graze damage). Used by applyBlueAdditionalEffect.
    params.bluePhysicalCleanHits = 0
    ...
    -- Failed accuracy rolls become partial ("graze") damage instead of a full miss.
    -- Tuned for low-level BLU; adjust if needed.
    local bluePhysGrazeMultiplier = 0.35
    ...
    while hitsdone < params.numhits do
        ...
        then
            ...
            params.bluePhysicalCleanHits = params.bluePhysicalCleanHits + 1
            ...
            -- Graze: same pDIF roll as a clean hit, no crit, scaled damage (does not count as TP multihit).
            local pdif = math.random(cratio[1] * 1000, cratio[2] * 1000)
            pdif       = pdif / 1000

            local hitDmg = 0
            if hitsdone == 0 then
                hitDmg = finalD * (multiplier + correlationMultiplier) * pdif * bluePhysGrazeMultiplier
            else
                hitDmg = finalD * (1 + correlationMultiplier) * pdif * bluePhysGrazeMultiplier
            end
    ...
    -- Avoid "no effect" from rounding / edge cases when the spell should still chip the target.
    -- (If finaldmg < 0, absorb/heal path in applySpellDamage handles it.)
    if finaldmg == 0 then
        finaldmg = 1
    end
...
xi.spells.blue.applyBlueAdditionalEffect = function(caster, target, params, effectTable)
    -- Physical BLU: only roll additional effects if at least one swing passed accuracy (graze-only does not).
    -- usePhysicalSpell sets bluePhysicalCleanHits; magical/breath spells do not set it (nil = no gate).
    if
        params.attackType == xi.attackType.PHYSICAL and
        params.bluePhysicalCleanHits ~= nil and
        params.bluePhysicalCleanHits == 0
    then
        return
    end
    ...
    for entry = 1, #effectTable do
        ...
        then
            local appliedDuration = math.floor(duration * resist)
            if target:addStatusEffect(effect, { power = power, duration = appliedDuration, origin = caster, tick = tick }) then
                xi.spells.debuffBattleLog.onDebuffApplied(caster, target, effect, power, appliedDuration)
            end
        end
    end
end
