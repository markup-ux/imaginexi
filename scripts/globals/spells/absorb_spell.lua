...
-- Party share: spell_list.animation column (client magic finish)
local drainAspirPartyAnim =
{
    [xi.magic.spell.DRAIN]     = 245,
    [xi.magic.spell.DRAIN_II]  = 246,
    [xi.magic.spell.DRAIN_III] = 246,
    [xi.magic.spell.ASPIR]     = 247,
    [xi.magic.spell.ASPIR_II]  = 248,
    [xi.magic.spell.ASPIR_III] = 288,
}

-- Caster already receives HP/MP in the main path; other party PCs get the same amount + action feedback.
local function shareDrainAspirToParty(caster, spell, amount, isHpDrain)
    if amount <= 0 then
        return
    end

    local spellId   = spell:getID()
    local animation = drainAspirPartyAnim[spellId]
    if not animation then
        return
    end

    local message = isHpDrain and xi.msg.basic.RECOVERS_HP or xi.msg.basic.RECOVERS_MP

    local recipients = {}

    local function addRecipient(ent)
        if not ent or ent:isDead() or ent:getObjType() ~= xi.objType.PC then
            return
        end

        if ent:getZoneID() ~= caster:getZoneID() or ent:getInstance() ~= caster:getInstance() then
            return
        end

        recipients[ent:getID()] = ent
    end
    ...
    for _, member in pairs(recipients) do
        if member:getID() ~= caster:getID() then
            local restored = isHpDrain and member:addHP(amount) or member:addMP(amount)
            if restored > 0 then
                caster:injectActionPacket(
                    member:getID(),
...
xi.spells.absorb.doDrainingSpell = function(caster, target, spell)
    ...
    if modAbsorbed == xi.mod.HP then
        ...
        shareDrainAspirToParty(caster, spell, finalDamage, true)
    else
        caster:addMP(finalDamage)
        target:delMP(finalDamage)
        shareDrainAspirToParty(caster, spell, finalDamage, false)
    end
