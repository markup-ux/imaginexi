...
namespace
{
enum class WeeklyBeastmanTribe : uint8
{
    Orcs = 0,
    Quadav,
    Yagudo,
...
uint32 getWeeklyTribeIndex()
{
    const auto nowSeconds = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    return static_cast<uint32>((nowSeconds / (60 * 60 * 24 * 7)) % 6);
}

WeeklyBeastmanTribe getWeeklyTribe()
{
    return static_cast<WeeklyBeastmanTribe>(getWeeklyTribeIndex());
}

bool mobMatchesWeeklyTribe(const CBaseEntity* PEntity)
{
    const auto* PMob = dynamic_cast<const CMobEntity*>(PEntity);
    if (!PMob)
    {
        return false;
    }

    std::string name = PMob->getName();
    std::transform(name.begin(), name.end(), name.begin(), [](unsigned char c)
    {
        return static_cast<char>(std::tolower(c));
    });

    const auto hasToken = [&](std::string_view token)
    {
        return name.find(token) != std::string::npos;
    };

    switch (getWeeklyTribe())
    {
        case WeeklyBeastmanTribe::Orcs:
            return hasToken("orc");
        case WeeklyBeastmanTribe::Quadav:
            return hasToken("quadav");
        case WeeklyBeastmanTribe::Yagudo:
            return hasToken("yagudo");
        case WeeklyBeastmanTribe::MamoolJa:
            return hasToken("mamool");
        case WeeklyBeastmanTribe::Trolls:
            return hasToken("troll");
        case WeeklyBeastmanTribe::Lamiae:
            return hasToken("lamia");
    }

    return false;
}

float getWeeklyBeastmanCurrencyMultiplier(const CBaseEntity* PEntity)
{
    const auto* PMob = dynamic_cast<const CMobEntity*>(PEntity);
    if (PMob && PMob->m_EcoSystem == ECOSYSTEM::BEASTMAN && mobMatchesWeeklyTribe(PMob))
    {
        return 1.5f;
    }
...
auto LoadChar(Scheduler& scheduler, MapConfig config, const uint32 charId) -> std::unique_ptr<CCharEntity>
{
    ...
    // Slot 0 must be gil; repair DB/tools that wrote normal items there before stats/zone scripts run.
    charutils::EnsureInventoryGilSlot(PChar);

    // Apply main.START_GIL in core: Lua charCreate only runs when `not zoning`; a stale char_stats.zoning
    // flag skips charCreate and leaves new characters at 0 gil despite settings.
    if (PChar->GetPlayTime(false) == std::chrono::seconds{0})
    {
        const uint32 startGil = static_cast<uint32>(std::max(0.0, settings::get<double>("main.START_GIL")));
        if (startGil > 0)
        {
            if (CItem* gilItem = PChar->getStorage(LOC_INVENTORY)->GetItem(0);
                gilItem != nullptr && gilItem->isType(ITEM_CURRENCY))
            {
                const uint32 curGil = gilItem->getQuantity();
                if (curGil < startGil)
                {
                    charutils::UpdateItem(PChar, LOC_INVENTORY, 0, static_cast<int32>(startGil - curGil));
                }
            }
            else
            {
                ShowWarning("LoadChar: could not apply main.START_GIL to charid %u (no currency in slot 0)", PChar->id);
            }
        }
    }
...
void LoadSpells(CCharEntity* PChar)
{
    ...
    GrantLevelEligibleSpells(PChar, true);
}

/************************************************************************
 *                                                                       *
 *  Grant spells from spell_list when main/sub job level meets the       *
 *  requirement (no scrolls). Skips trusts, merit spells, and disabled   *
 *  content. Blue magic uses this same path (mob learn disabled).        *
 *                                                                       *
 ************************************************************************/

void GrantLevelEligibleSpells(CCharEntity* PChar, bool suppressLearnMessages)
{
    TracyZoneScoped;

    bool learnedAny  = false;
    bool learnedBlue = false;

    const auto  mainJob    = PChar->GetMJob();
    const auto  subJob     = PChar->GetSJob();
    const uint8 mainJobLvl = PChar->jobs.job[mainJob];
    const uint8 subLvl     = PChar->GetSLevel();
    ...
    for (uint16 spellId = 1; spellId < MAX_SPELL_ID; ++spellId)
    {
        ...
        if (PSpell->getMeritId() != 0)
        {
            continue;
        }

        const uint8 requirements = PSpell->getRequirements();
        if (requirements & SPELLREQ_MERIT)
        {
            continue;
        }
        ...
        if (!eligible && subJob != JOB_NON && !(requirements & SPELLREQ_MAIN_JOB_ONLY))
        {
        ...
        if (addSpell(PChar, spellId, !suppressLearnMessages))
        {
            SaveSpell(PChar, spellId);
            learnedAny = true;

            if (group == SPELLGROUP_BLUE)
            {
                learnedBlue = true;
            }
        }
    ...
    if (learnedBlue && (PChar->GetMJob() == JOB_BLU || PChar->GetSJob() == JOB_BLU))
    {
...
void LoadInventory(CCharEntity* PChar)
{
    ...
    if (rset && rset->rowsCount())
    {
        while (rset->next())
        {
            ...
            // Legacy SQL default used Gil's item id (65535) as an "empty" sentinel with qty 0.
            // xi::items::spawn(0xFFFF) builds a bare CItemCurrency; skip so the slot stays empty.
...
/************************************************************************
 *                                                                       *
 *  Ensure gil currency exists in inventory slot 0                       *
 *                                                                       *
 ************************************************************************/

void EnsureInventoryGilSlot(CCharEntity* PChar)
{
    CItem* existing = PChar->getStorage(LOC_INVENTORY)->GetItem(0);
    if (existing != nullptr)
    {
        if (existing->isType(ITEM_CURRENCY))
        {
            return;
        }
        ShowWarning(
            "EnsureInventoryGilSlot: inventory slot 0 is not currency (charid %u, item %u); moving to first free slot",
            PChar->id,
            existing->getID());
        if (MoveItem(PChar, LOC_INVENTORY, 0, ERROR_SLOTID) == ERROR_SLOTID)
        {
            ShowError(
                "EnsureInventoryGilSlot: could not vacate slot 0 for gil (charid %u); inventory may be full",
                PChar->id);
            return;
        }
        existing = PChar->getStorage(LOC_INVENTORY)->GetItem(0);
    }

    if (existing != nullptr)
    {
        ShowError("EnsureInventoryGilSlot: slot 0 still occupied after repair (charid %u)", PChar->id);
        return;
    }

    auto PGil = xi::items::spawn(0xFFFF);
    if (PGil == nullptr || !PGil->isType(ITEM_CURRENCY))
    {
        ShowError("EnsureInventoryGilSlot: failed to spawn gil for charid %u", PChar->id);
        return;
    }

    PGil->setQuantity(0);
    PGil->setLocationID(LOC_INVENTORY);
    PGil->setSlotID(0);

    // Slot 0 may already exist in SQL with itemId 0 / legacy sentinels that LoadInventory never loads.
    ...
    if (!db::preparedStmt(Query, PChar->id, LOC_INVENTORY, 0, PGil->getID(), PGil->getQuantity(), PGil->getSignature(), PGil->m_extra))
    {
        ShowError("EnsureInventoryGilSlot: upsert failed for charid %u", PChar->id);
        return;
    }

    PChar->getStorage(LOC_INVENTORY)->InsertItem(std::move(PGil), 0);

    if (CItem* inserted = PChar->getStorage(LOC_INVENTORY)->GetItem(0))
    {
        PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(inserted, LOC_INVENTORY, 0);
        PChar->pushPacket<GP_SERV_COMMAND_ITEM_SAME>(PChar);
    }
}
...
uint8 AddItem(CCharEntity* PChar, uint8 LocationID, uint16 ItemID, uint32 quantity, bool silence)
{
    ...
    // Gil uses fixed slot 0; do not require a free generic slot (inventory can read "full" while currency is missing).
    if (!PItem->isType(ITEM_CURRENCY) && PChar->getStorage(LocationID)->GetFreeSlotsCount() == 0)
    {
        return ERROR_SLOTID;
    }
...
auto AddItem(CCharEntity* PChar, uint8 LocationID, std::unique_ptr<CItem> PItem, bool silence) -> uint8
{
    ...
    {
        EnsureInventoryGilSlot(PChar);
        UpdateItem(PChar, LocationID, 0, PItem->getQuantity());
        return 0;
    }

    if ((PItem->hasFlag(ItemFlag::Rare) || PItem->hasFlag(ItemFlag::Exclusive)) && HasItem(PChar, PItem->getID()))
    {
        if (!silence)
        {
...
void UpdateSubJob(CCharEntity* PChar)
{
    ...
    charutils::BuildingCharAbilityTable(PChar, true);
    charutils::BuildingCharTraitsTable(PChar, true);
    ...
    GrantLevelEligibleSpells(PChar);
}
                ...
                else
                {
                    switch (weapon->getSkillType())
                    {
                        ...
                        case SKILL_CLUB:
                        {
                            ...
                            if (isWeapon && PChar->objtype != TYPE_PC && !charutils::hasDualWieldAbility(PChar))
                            {
                                return false;
                            }

                            if (isWeapon && PNewItemWeapon && PNewItemWeapon->getSkillType() == SKILL_NONE)
                            {
                                return false;
                            }
                        ...
                        default:
                        {
                            if (PChar->objtype == TYPE_PC && charutils::hasDualWieldAbility(PChar) && PItem->isType(ITEM_WEAPON))
                            {
                                auto* PNewItemWeapon = dynamic_cast<CItemWeapon*>(PItem);
                                if (PNewItemWeapon && PNewItemWeapon->getSkillType() != SKILL_NONE && !weapon->isTwoHanded())
                                {
                                    PChar->m_Weapons[SLOT_SUB] = PItem;
                                    PChar->m_dualWield         = true;
                                    break;
...
void EquipItem(CCharEntity* PChar, uint8 slotID, uint8 equipSlotID, uint8 containerID)
{
    ...
    if (equipSlotID == SLOT_SUB && PItem && !PItem->IsShield())
    {
        ...
        {
            // Imagine XI: all player jobs may equip sub-weapons from level 1; skip retail Dual Wield gate.
            ...
            if (!PChar->getEquip(SLOT_MAIN) || (PMainItem && PMainItem->isTwoHanded()))
            {
                return;
            }
        }
    ...
    {
        if ((PItem != nullptr) && (PItem->isType(ITEM_EQUIPMENT) || PItem->isType(ITEM_WEAPON)))
        {
...
void CheckValidEquipment(CCharEntity* PChar)
{
    ...
    for (uint8 slotID = 0; slotID < 16; ++slotID)
    {
        ...
        {
            // Unequip if no main weapon or a non-grip subslot without DW (players always allowed)
            if (!PChar->getEquip(SLOT_MAIN) ||
                (PChar->objtype != TYPE_PC && !charutils::hasDualWieldAbility(PChar) &&
...
namespace
{
void AppendCharPetCommandsForEntity(CCharEntity* PChar, CBattleEntity* petSource)
{
    if (petSource == nullptr || PChar == nullptr)
    {
        return;
    }

    if (auto* PPet = dynamic_cast<CPetEntity*>(petSource))
    {
        const uint32 PetID = PPet->m_PetID;
        if (PetID == 0)
        {
            return;
        }
    ...
    if (petSource->objtype == TYPE_MOB && petSource->isCharmed)
    {
        auto skillList{ battleutils::GetMobSkillList(static_cast<CMobEntity*>(petSource)->m_MobSkillList) };
        for (auto&& abilityid : skillList)
        {
            addPetAbility(PChar, abilityid - ABILITY_HEALING_RUBY);
        }
...
void BuildingCharPetAbilityTable(CCharEntity* PChar, CPetEntity* PPet, uint32 PetID)
{
    ...
    AppendCharPetCommandsForEntity(PChar, PPet);
    PChar->pushPacket<GP_SERV_COMMAND_COMMAND_DATA>(PChar);
}

void RebuildCharPetAbilityTable(CCharEntity* PChar)
{
    if (PChar == nullptr)
    {
        return;
    ...
    std::memset(&PChar->m_PetCommands, 0, sizeof(PChar->m_PetCommands));
    for (CBattleEntity* pet : PChar->GetAllPets())
    {
        AppendCharPetCommandsForEntity(PChar, pet);
    }
    PChar->pushPacket<GP_SERV_COMMAND_COMMAND_DATA>(PChar);
}
...
void BuildingCharAbilityTable(CCharEntity* PChar, bool announceNewAbilities)
{
    if (PChar == nullptr)
    {
    ...
    auto pushAbilityLearnChat = [PChar](uint16 abilityId)
    {
        if (PChar->loc.zone == nullptr)
        {
            return;
        }
        CAbility* PAb = ability::GetAbility(abilityId);
        const std::string label = PAb ? spell::FormatScriptNameForDisplay(PAb->getName()) : fmt::format("ability #{}", abilityId);
        const std::string msg   = fmt::format("You learn the ability {}.", label);
        PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF,
                                    std::make_unique<GP_SERV_COMMAND_CHAT_STD>("", PChar->getZone(), MESSAGE_SYSTEM_1, msg, 0));
    };

    uint8_t oldAbilities[64];
    std::memcpy(oldAbilities, PChar->m_Abilities, sizeof(oldAbilities));

    std::memset(&PChar->m_Abilities, 0, sizeof(PChar->m_Abilities));

    // Begin: Force Provoke for PLD, RUN, NIN, and MNK (main or sub, level 1+).
    // Ensures these jobs can use enmity control from job level 1 regardless of default ability table unlocks.
    do
    {
        const bool mainIsProvoker = (PChar->GetMJob() == JOB_PLD || PChar->GetMJob() == JOB_RUN || PChar->GetMJob() == JOB_NIN ||
                                     PChar->GetMJob() == JOB_MNK);
        const bool subIsProvoker = (PChar->GetSJob() == JOB_PLD || PChar->GetSJob() == JOB_RUN || PChar->GetSJob() == JOB_NIN ||
                                    PChar->GetSJob() == JOB_MNK);
        if ((mainIsProvoker && PChar->GetMLevel() >= 1) || (subIsProvoker && PChar->GetSLevel() >= 1))
        {
            const bool isNew = !hasBit(ABILITY_PROVOKE, oldAbilities, sizeof(oldAbilities));
            addAbility(PChar, ABILITY_PROVOKE);
            if (announceNewAbilities && isNew)
            {
                pushAbilityLearnChat(ABILITY_PROVOKE);
            }
        }
    } while (0);
    // End: Force Provoke for PLD/RUN/NIN/MNK
    ...
    for (auto PAbility : ability::GetAbilities(PChar->GetMJob()))
    {
        ...
        if (PChar->GetMLevel() >= PAbility->getLevel())
        {
            ...
            {
                const uint16 aid = PAbility->getID();
                const bool   isNew = !hasBit(aid, oldAbilities, sizeof(oldAbilities));
                addAbility(PChar, aid);
...
bool IsCombatMagicAutoCapSkill(uint8 skillId)
{
    if (skillId < 1 || skillId >= 48)
    {
        return false;
    }
...
uint16 GetCombatMagicSkillCap(CCharEntity* PChar, SKILLTYPE skillId)
{
    if (!IsCombatMagicAutoCapSkill(static_cast<uint8>(skillId)))
    {
        return 0;
    }

    uint8 const realMLv = PChar->jobs.job[PChar->GetMJob()];
    uint8 const realSLv = PChar->jobs.job[PChar->GetSJob()];
    ...
    if (skillId >= SKILL_AUTOMATON_MELEE && skillId <= SKILL_AUTOMATON_MAGIC)
    {
        if (PChar->GetMJob() == JOB_PUP)
        {
            maxMainSkill = battleutils::GetMaxSkill(1, realMLv);
        }
        else if (PChar->GetSJob() == JOB_PUP)
        {
...
// Persist combat + magic skills to caps for stored main/sub job levels (not effective/synced MLV).
// Crafts, fishing, RID, DIG unchanged. Avoids lowering real skills when temporarily level-synced down.
void RaiseCombatMagicSkillsToRealJobCaps(CCharEntity* PChar)
{
    if (PChar->GetMJob() == JOB_MON || PChar->GetSJob() == JOB_MON)
    {
        return;
    }
    ...
    for (int32 i = 1; i < 48; ++i)
    {
        ...
        uint16 const cap = GetCombatMagicSkillCap(PChar, static_cast<SKILLTYPE>(i));
        if (cap == 0)
        {
            continue;
        }

        uint16 const targetTenths = static_cast<uint16>(cap * 10);
        uint16 const prev           = PChar->RealSkills.skill[i];
        if (prev != targetTenths)
        {
            PChar->RealSkills.skill[i] = targetTenths;
            SaveCharSkills(PChar, static_cast<uint8>(i));
        }
    }
}
...
{
    RaiseCombatMagicSkillsToRealJobCaps(PChar);
    ...
    const bool levelSyncActive = PChar->StatusEffectContainer->HasStatusEffect(EFFECT_LEVEL_SYNC);
    ...
    {
        int32 skillValue = static_cast<int32>((PChar->RealSkills.skill[i] / 10) * 0x20) + PChar->RealSkills.rank[i];

        if (i <= SKILL_COOKING)
        {
            skillValue += PChar->getMod(static_cast<Mod>(i + 79)) * 0x20;
        }
        ...
        PChar->WorkingSkills.skill[i] = static_cast<uint16>(skillValue);
...
static void EnsureUniversalDualWield(CCharEntity* PChar, uint8 mjob, uint8 sjob, uint8 mlvl, uint8 slvl);

void BuildingCharTraitsTable(CCharEntity* PChar, bool announceNewTraits)
{
    std::set<std::pair<uint16, uint8>> oldTraitKeys;
    for (CTrait* PTrait : PChar->TraitList)
    {
        oldTraitKeys.emplace(PTrait->getID(), PTrait->getRank());
    }
    ...
    EnsureUniversalDualWield(PChar, mjob, sjob, mlvl, slvl);

    // Refresh client trait bitmask (0x0AC) after every rebuild; required for non-NIN/THF/DNC dual wield.
    if (PChar->objtype == TYPE_PC && mlvl >= 1 && PChar->loc.zone)
    {
        PChar->pushPacket<GP_SERV_COMMAND_COMMAND_DATA>(PChar);
    }
    ...
    if (announceNewTraits && PChar->loc.zone)
    {
        std::set<std::pair<uint16, uint8>> announcedTraitKeys;
        for (CTrait* PTrait : PChar->TraitList)
        {
            const auto key = std::make_pair(PTrait->getID(), PTrait->getRank());
            if (!oldTraitKeys.count(key) && announcedTraitKeys.emplace(key).second)
            {
                std::string traitLabel = PTrait->getName();
                if (traitLabel.empty())
                {
                    traitLabel = fmt::format("trait #{}", PTrait->getID());
                }
                else
                {
                    traitLabel = spell::FormatScriptNameForDisplay(traitLabel);
                }
                const std::string msg = fmt::format("You gain the trait: {} (rank {}).", traitLabel, PTrait->getRank());
                PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF,
                                            std::make_unique<GP_SERV_COMMAND_CHAT_STD>("", PChar->getZone(), MESSAGE_SYSTEM_1, msg, 0));
            }
        }
...
void TrySkillUP(CCharEntity* PChar, SKILLTYPE SkillID, uint8 lvl, bool forceSkillUp, bool useSubSkill)
{
    ...
    // Imagine XI: combat and magic skills are always at job cap; no retail skill-up progression.
    if (IsCombatMagicAutoCapSkill(rawSkillID) && PChar->GetMJob() != JOB_MON && PChar->GetSJob() != JOB_MON)
    {
        return;
    }
...
void CheckWeaponSkill(CCharEntity* PChar, uint8 skill)
{
    ...
    for (auto&& PSkill : WeaponSkillList)
    {
        if (curSkill == PSkill->getSkillLevel() && (battleutils::CanUseWeaponskill(PChar, PSkill)))
        {
            ...
            if (PChar->loc.zone)
            {
                const std::string msg = fmt::format("You learn the weaponskill {}.", PSkill->getName());
                PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF,
                                            std::make_unique<GP_SERV_COMMAND_CHAT_STD>("", PChar->getZone(), MESSAGE_SYSTEM_1, msg, 0));
            }
            PChar->pushPacket<GP_SERV_COMMAND_COMMAND_DATA>(PChar);
...
int32 addSpell(CCharEntity* PChar, uint16 spellID, bool showLearnMessage)
{
    auto* PSpell = spell::GetSpell(static_cast<SpellID>(spellID));
    if (PSpell && !hasSpell(PChar, spellID))
    {
        PChar->m_SpellList[spellID] = true;
        if (showLearnMessage && PChar->loc.zone)
        {
            const std::string msg = fmt::format("You learn the spell {}.", spell::FormatScriptNameForDisplay(PSpell->getName()));
            PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF,
                                        std::make_unique<GP_SERV_COMMAND_CHAT_STD>("", PChar->getZone(), MESSAGE_SYSTEM_1, msg, 0));
        }
        return 1;
...
bool canUseWeaponSkill(CCharEntity* PChar, uint16 wsid)
{
    ...
    if (!hasWeaponSkill(PChar, wsid))
    {
        return false;
    }

    // Mythic / skill-gated: CanUseWeaponskill enforces skill, unlock, and max-level rules.
    // Relic / latent (skill 0, unlock 0): allowed only if the WS is already on the character (from ADDS_WEAPONSKILL).
    if (battleutils::CanUseWeaponskill(PChar, PWeaponSkill))
    {
        return true;
    }

    return PWeaponSkill->getSkillLevel() == 0 && PWeaponSkill->getUnlockId() == 0;
}
...
bool hasDualWieldAbility(CCharEntity* PChar)
{
    if (PChar == nullptr)
    {
        return false;
    ...
    // Imagine XI: all player jobs can dual wield from level 1.
    if (PChar->objtype == TYPE_PC && PChar->GetMLevel() >= 1)
    {
        return true;
    }

    if (PChar->hasTrait(TRAIT_DUAL_WIELD) || hasTrait(PChar, TRAIT_DUAL_WIELD))
    {
        return true;
    }

    return PChar->getMod(Mod::DUAL_WIELD) > 0;
}

static void EnsureUniversalDualWield(CCharEntity* PChar, uint8 mjob, uint8 sjob, uint8 mlvl, uint8 slvl)
{
    if (PChar == nullptr || PChar->objtype != TYPE_PC || mlvl < 1)
    {
        return;
    }
    ...
    {
        const std::array<std::pair<uint8, uint8>, 2> jobs = { { { mjob, mlvl }, { sjob, slvl } } };
        for (const auto& [jobId, level] : jobs)
        {
            if (jobId == 0 || jobId >= MAX_JOBTYPE || level == 0)
            {
                continue;
            }

            auto* traitList = traits::GetTraits(jobId);
            ...
            {
                if (PTrait->getID() == TRAIT_DUAL_WIELD && level >= PTrait->getLevel() && PTrait->getLevel() > 0)
                {
                    PChar->addTrait(PTrait);
                    break;
                }
            }
    ...
    // Always sync m_TraitList bit 18 for 0x0AC. THF/NIN/DNC bypass client-side via job id; all other jobs need this bit.
    addTrait(PChar, TRAIT_DUAL_WIELD);

    if (PChar->getMod(Mod::DUAL_WIELD) == 0)
    {
        PChar->addModifier(Mod::DUAL_WIELD, 35);
    }
}
...
bool HasKillCredit(CCharEntity* PChar, CMobEntity* PMob, float range)
{
    TracyZoneScoped;
    ...
    if (CheckMob(PMob->m_HiPCLvl, PMob) > EMobDifficulty::TooWeak &&
        distance(PMob->loc.p, PChar->loc.p) < range &&
        !PMob->GetCallForHelpFlag())
    {
...
void DistributeGil(CCharEntity* PChar, CMobEntity* PMob)
{
    ...
    if (PChar->PParty != nullptr)
    {
        ...
        if (!members.empty())
        {
            ...
            {
                EnsureInventoryGilSlot(PMember);
...
void AddExperiencePoints(bool expFromRaise, CCharEntity* PChar, CBaseEntity* PMob, uint32 exp, EMobDifficulty mobCheck, bool isexpchain)
{
    ...
    if (!expFromRaise)
    {
        ...
        const float weeklyCurrencyMultiplier = getWeeklyBeastmanCurrencyMultiplier(PMob);
        const int32 rewardExpForCurrency     = static_cast<int32>(std::floor(exp * weeklyCurrencyMultiplier));
        ...
        if (PChar->StatusEffectContainer->HasStatusEffect(EFFECT_SIGNET) && (region >= REGION_TYPE::RONFAURE && region <= REGION_TYPE::JEUNO))
        {
            ...
            conquest::AddConquestPoints(PChar, rewardExpForCurrency);
        }
        ...
        {
            charutils::AddPoints(PChar, "imperial_standing", static_cast<int32>(rewardExpForCurrency * 0.1f));
    ...
    if ((currentExp + exp) >= GetExpNEXTLevel(PChar->jobs.job[PChar->GetMJob()]) && !onLimitMode)
    {
        ...
        else
        {
            ...
            if (PChar->m_LevelRestriction == 0 || PChar->m_LevelRestriction > PChar->GetMLevel())
            {
                ...
                BuildingCharSkillsTable(PChar);
                CalculateStats(PChar);
                BuildingCharAbilityTable(PChar, true);
                BuildingCharTraitsTable(PChar, true);
                BuildingCharWeaponSkills(PChar);
                puppetutils::LoadAutomaton(PChar);
            }
...
auto CheckAbilityAddtype(CCharEntity* PChar, const CAbility* PAbility) -> bool
{
    ...
    if ((PAbility->getAddType() & (ADDTYPE_JUGPET | ADDTYPE_CHARMPET)) == (ADDTYPE_JUGPET | ADDTYPE_CHARMPET))
    {
        ...
        for (CBattleEntity* pet : PChar->GetAllPets())
        {
            if (pet && (pet->objtype == TYPE_MOB ||
                        (pet->objtype == TYPE_PET && static_cast<CPetEntity*>(pet)->getPetType() == PET_TYPE::JUG_PET)))
            {
                ok = true;
                break;
            }
    ...
    if ((PAbility->getAddType() & (ADDTYPE_JUGPET | ADDTYPE_CHARMPET)) == ADDTYPE_JUGPET)
    {
        ...
        for (CBattleEntity* pet : PChar->GetAllPets())
        {
            if (pet && pet->objtype == TYPE_PET && static_cast<CPetEntity*>(pet)->getPetType() == PET_TYPE::JUG_PET)
            {
                ok = true;
                break;
            }
    ...
    {
        CPetEntity const* petEntity = nullptr;
        for (CBattleEntity* pet : PChar->GetAllPets())
        {
            if (pet && pet->objtype == TYPE_PET && static_cast<CPetEntity*>(pet)->getPetType() == PET_TYPE::AVATAR)
            {
                petEntity = static_cast<CPetEntity*>(pet);
                break;
            }
        }
    ...
    if (PAbility->getAddType() & ADDTYPE_AUTOMATON)
    {
        ...
        for (CBattleEntity* pet : PChar->GetAllPets())
        {
            if (pet && pet->objtype == TYPE_PET && static_cast<CPetEntity*>(pet)->getPetType() == PET_TYPE::AUTOMATON)
            {
                ok = true;
                break;
            }
...
void RemoveStratagems(CCharEntity* PChar, CSpell* PSpell)
{
    if (PSpell->getSpellGroup() == SPELLGROUP_WHITE)
    {
        ...
        // ImagineXI: Accession is a timed window (JA duration), not consumed per eligible spell — same idea as BLU Diffusion.
    ...
    else if (PSpell->getSpellGroup() == SPELLGROUP_BLACK)
    {
        ...
        // ImagineXI: Manifestation is a timed window (JA duration), not consumed per eligible spell — same idea as BLU Diffusion.
    }
}
...
void ReloadParty(CCharEntity* PChar)
{
    ...
    FOR_DB_SINGLE_RESULT(rset)
    {
        ...
        {
            auto*       PSyncChar = static_cast<CCharEntity*>(PSyncTarget);
            const uint8 syncPow   = std::min<uint8>(PSyncChar->jobs.job[PSyncChar->GetMJob()], settings::get<uint8>("main.MAX_LEVEL"));
            PChar->pushPacket<GP_SERV_COMMAND_BATTLE_MESSAGE>(PChar, PChar, 0, syncPow, MsgBasic::LevelSyncActivated);
            PChar->StatusEffectContainer->DelStatusEffectsByFlag(EFFECTFLAG_DISPELABLE);
            PChar->StatusEffectContainer->AddStatusEffect(new CStatusEffect(EFFECT_LEVEL_SYNC, EFFECT_LEVEL_SYNC, syncPow, 0s, 0s), EffectNotice::Silent);
        }
...
void PolExit(CCharEntity* PChar)
{
    TracyZoneScoped;
    ...
    PChar->pushPacket<GP_SERV_COMMAND_LOGOUT>(GP_GAME_LOGOUT_STATE::POLEXIT, IPP());
}
...
namespace
{
    // item_mods.modId for Mod::EQUIPMENT_ONLY_RACE
    constexpr uint16 EQUIPMENT_ONLY_RACE_MODID = 276;

    uint16 FindEquipmentModelIdForRaceVariant(uint16 equippedItemId, uint8 previewRace)
    {
        if (equippedItemId == 0 || previewRace < 1 || previewRace > 8)
        {
            return 0;
        }

        const auto rset = db::preparedStmt(
            "SELECT ie2.MId AS modelId "
            "FROM item_equipment AS ie1 "
            "INNER JOIN item_basic AS b1 ON b1.itemid = ie1.itemId "
            "INNER JOIN item_basic AS b2 ON b2.subid = b1.subid AND b2.itemid <> b1.itemid "
            "INNER JOIN item_equipment AS ie2 ON ie2.itemId = b2.itemid "
            "LEFT JOIN item_mods AS im ON im.itemId = ie2.itemId AND im.modId = ? "
            "WHERE ie1.itemId = ? AND b1.subid <> 0 "
            "AND ie1.slot = ie2.slot AND ie1.ilevel = ie2.ilevel "
            "AND (im.value IS NULL OR im.value = 0 OR (im.value & (1 << (? - 1))) <> 0) "
            "ORDER BY ie2.itemId ASC "
            "LIMIT 1",
            EQUIPMENT_ONLY_RACE_MODID,
            equippedItemId,
            previewRace);

        if (rset && rset->next())
        {
            return rset->get<uint16>("modelId");
        }
        return 0;
    }
} // namespace
...
uint16 charutils::GetAppearanceEquipmentModelForRace(CCharEntity* PChar, uint8 equipSlot, uint8 previewRace)
{
    ...
    const auto slot = static_cast<SLOTTYPE>(equipSlot);
    ...
    // Hand-to-hand: sub appearance comes from main weapon model + 0x1000 (matches char equip logic).
    if (slot == SLOT_SUB)
    {
        if (auto* mainW = dynamic_cast<CItemWeapon*>(PChar->getEquip(SLOT_MAIN));
            mainW && mainW->getSkillType() == SKILL_HAND_TO_HAND)
        {
            if (mainW->isEquippableByRace(previewRace))
            {
                return mainW->getModelId() + 0x1000;
            }
            const uint16 v = FindEquipmentModelIdForRaceVariant(mainW->getID(), previewRace);
            if (v != 0)
            {
                return v + 0x1000;
            }
    ...
    if (PItem->isType(ITEM_WEAPON))
    {
        auto* weap = static_cast<CItemWeapon*>(PItem);
        if (weap->isEquippableByRace(previewRace))
        {
            return weap->getModelId();
        }
        const uint16 v = FindEquipmentModelIdForRaceVariant(weap->getID(), previewRace);
        return v != 0 ? v : weap->getModelId();
    }

    if (PItem->isType(ITEM_EQUIPMENT))
    {
        auto* eq = static_cast<CItemEquipment*>(PItem);
        if (eq->isEquippableByRace(previewRace))
        {
            return eq->getModelId();
        }
        const uint16 v = FindEquipmentModelIdForRaceVariant(eq->getID(), previewRace);
        return v;
    }
