...
xi.spells.enhancing.calculateEnhancingDuration = function(caster, target, spell, spellId, spellGroup, spellEffect)
    ...
    -- Active EXP chain bonus to all enhancing duration.
    -- Guard against older map binaries that may not expose getExpChainCount to Lua.
    if caster.getExpChainCount ~= nil and caster:isPC() and caster:getExpChainCount() > 0 then
        local chainBonus = (xi.settings.map and xi.settings.map.EXP_CHAIN_ENHANCING_DURATION_BONUS) or 0
        duration = duration + duration * chainBonus / 100
    end
...
-- Main WHM: bar-element spells also grant the matching enspell on targets they land on.
local barElementEnspellSpellId =
{
    [xi.magic.spell.BARFIRE    ] = xi.magic.spell.ENFIRE,
    [xi.magic.spell.BARBLIZZARD] = xi.magic.spell.ENBLIZZARD,
    [xi.magic.spell.BARAERO    ] = xi.magic.spell.ENAERO,
    [xi.magic.spell.BARSTONE   ] = xi.magic.spell.ENSTONE,
    [xi.magic.spell.BARTHUNDER ] = xi.magic.spell.ENTHUNDER,
    [xi.magic.spell.BARWATER   ] = xi.magic.spell.ENWATER,
    [xi.magic.spell.BARFIRA    ] = xi.magic.spell.ENFIRE_II,
    [xi.magic.spell.BARBLIZZARA] = xi.magic.spell.ENBLIZZARD_II,
    [xi.magic.spell.BARAERA    ] = xi.magic.spell.ENAERO_II,
    [xi.magic.spell.BARSTONRA  ] = xi.magic.spell.ENSTONE_II,
    [xi.magic.spell.BARTHUNDRA ] = xi.magic.spell.ENTHUNDER_II,
    [xi.magic.spell.BARWATERA  ] = xi.magic.spell.ENWATER_II,
}

local function applyMainWhmBarElementEnspell(caster, target, spell, barSpellId)
    if not caster:isPC() or caster:getMainJob() ~= xi.job.WHM then
        return
    end

    local enSpellId = barElementEnspellSpellId[barSpellId]
    if not enSpellId then
        return
    end

    local enParams   = pTable[enSpellId]
    local enEffect   = enParams[column.EFFECT_ID]
    local enTier     = enParams[column.EFFECT_TIER]
    local enTickTime = enParams[column.EFFECT_TICK_RATE]
    local spellGroup = spell:getSpellGroup()

    local basePower  = xi.spells.enhancing.calculateEnhancingBasePower(caster, target, spell, enSpellId, enEffect)
    local finalPower = xi.spells.enhancing.calculateEnhancingFinalPower(caster, target, spell, basePower, spellGroup, enTier, enEffect)
    local duration   = xi.spells.enhancing.calculateEnhancingDuration(caster, target, spell, enSpellId, spellGroup, enEffect)

    target:addStatusEffect(enEffect, { power = finalPower, duration = duration, origin = caster, tick = enTickTime, tier = enTier })
end
...
xi.spells.enhancing.useEnhancingSpell = function(caster, target, spell)
    ...
    ------------------------------------------------------------
    local isBarElement = spellEffect >= xi.effect.BARFIRE and spellEffect <= xi.effect.BARWATER
    ...
    if alwaysOverwrite then
        ...
        if isBarElement then
            applyMainWhmBarElementEnspell(caster, target, spell, spellId)
        end
    else
