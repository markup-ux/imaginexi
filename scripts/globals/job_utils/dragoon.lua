...
xi.job_utils.dragoon.abilityCheckRequiresPet = function(player, target, ability, checkActionable)
    if not hasWyvern(player) then
        ...
        if checkActionable and wyvern and not wyvern:canUseAbilities() then
...
xi.job_utils.dragoon.useAncientCircle = function(player, target, ability)
    ...
    local duration            = longCircleDurationS + player:getMod(xi.mod.ANCIENT_CIRCLE_DURATION)
    ...
    target:addStatusEffect(xi.effect.ANCIENT_CIRCLE, { power = power, subPower = subPower, duration = duration, origin = player })
...
xi.job_utils.dragoon.pickAndUseDamageBreath = function(player, target)
    local wyvernPet = getWyvern(player)
    if not wyvernPet then
        return
    end
    ...
    wyvernPet:usePetAbility(breathToUse, target)
end
