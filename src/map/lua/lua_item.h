...
public:
    ...
    uint32 getJobs();                // get the jobs bitmask (equipment only)
    uint16 getEquipSlotId();         // get the equip slot bitmask (equipment only)
    uint8  getSuperiorLevel();       // get the superior (job master) level requirement
    uint16 getWeaponDamage();        // get weapon damage (weapons only)
    uint16 getWeaponDelay();         // get weapon delay (weapons only)
