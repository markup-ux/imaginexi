...
void CCharEntity::setPetZoningInfo()
{
    ...
    auto capturePet = [&](CBattleEntity* PPet, PetInfo_t& info)
    {
        ...
        switch (PPetEntity->getPetType())
        {
            case PET_TYPE::JUG_PET:
                ...
                info.jugSpawnTime = PPetEntity->getJugSpawnTime();
                info.jugDuration  = PPetEntity->getJugDuration();
                [[fallthrough]];
            ...
            case PET_TYPE::WYVERN:
                info.petLevel     = PPetEntity->getSpawnLevel();
                info.petHP        = PPet->health.hp;
                info.petTP        = PPet->health.tp;
                info.petMP        = PPet->health.mp;
                info.petType      = PPetEntity->getPetType();
                info.respawnPet   = true;
                break;
            default:
                break;
        }
    ...
    for (CBattleEntity* PPet : GetAllPets())
    {
        if (PPet == nullptr || PPet->objtype != TYPE_PET)
        {
            continue;
        }
        capturePet(PPet, petZoningInfo);
        break;
    }
}
...
{
    auto persistType = [](PET_TYPE petType) -> bool
    {
    ...
    if (petZoningInfo.respawnPet && persistType(petZoningInfo.petType))
    {
        return true;
    }
    PET_TYPE petType{};
...
{
    uint16 const prevTp = health.tp;
    tp                  = CBattleEntity::addTP(tp);
    PLatentEffectContainer->CheckLatentsTP();
    battleutils::OnPlayerTpChanged(this, prevTp);
...
{
    TracyZoneScoped;
    auto spellId = static_cast<uint16>(PSpell->getID());

    // Virus/Curse: allow cast when job/level permits even without scroll learn (no stable learn source yet).
    if (spellId == 256 || spellId == 257)
    {
        return CBattleEntity::CanUseSpell(PSpell);
    }

    return charutils::hasSpell(this, spellId) && CBattleEntity::CanUseSpell(PSpell);
}
...
void CCharEntity::OnCastFinished(CMagicState& state, action_t& action)
{
    ...
    // Elemental Seal is cleared by EFFECTFLAG_MAGIC_END in the base cast handler, but skillchain
    // processing runs afterward, so capture the buff here first.
    const bool hasElementalSealForSkillchain = StatusEffectContainer->HasStatusEffect(EFFECT_ELEMENTAL_SEAL);
    ...
    for (auto&& actionTarget : action.targets)
    {
        for (auto&& actionResult : actionTarget.results)
        {
            ...
            // Immanence and Elemental Seal will create or extend a skillchain for elemental spells
            if (PTarget->health.hp > 0 &&
                actionResult.param >= 0 &&
                PSpell->dealsDamage() &&
                PSpell->getSpellGroup() == SPELLGROUP_BLACK &&
                (StatusEffectContainer->HasStatusEffect(EFFECT_IMMANENCE) || hasElementalSealForSkillchain))
            {
...
void CCharEntity::OnWeaponSkillFinished(CWeaponSkillState& state, action_t& action)
{
    ...
    float wsMaxDist = static_cast<float>(PWeaponSkill->getRange());
    if (battleutils::isMagicRangeStaffElementalWeaponskill(PWeaponSkill->getID()))
    {
        wsMaxDist = battleutils::getMagicRangeStaffWeaponskillMaxDistance();
    }

    if (distance(loc.p, PBattleTarget->loc.p) <= wsMaxDist + PBattleTarget->modelHitboxSize + modelHitboxSize)
    {
...
void CCharEntity::OnRangedAttack(CRangeState& state, action_t& action)
{
    ...
    const bool infiniteAmmo = settings::get<bool>("map.DISABLE_AMMO_CONSUMPTION");
    ...
    else if (ammoThrowing && isSange)
    {
        ...
        if (!infiniteAmmo && PAmmo && PAmmo->getQuantity() < hitCount)
        {
    ...
    for (uint8 i = 1; i <= hitCount; ++i)
    {
        ...
        if (!infiniteAmmo && PAmmo != nullptr && xirand::GetRandomNumber(100) > recycleChance)
        {
            ++ammoConsumed;
...
void CCharEntity::OnRaise()
{
    ...
    if (m_hasRaise > 0)
    {
        ...
        // Pixie rescue: local vars must match scripts/globals/pixie_rescue.lua (TICKET / POST_REVIVE_HANDLED).
        // Snap HP/MP to current max (weakness already applied) before MagicFinish so no aggro tick sees sub-75% HPP.
        if (GetLocalVar("PIXIE_RESCUE_TICKET") != 0U && GetLocalVar("PIXIE_RESCUE_POST_DONE") == 0U)
        {
            const int32 maxHp = GetMaxHP();
            if (maxHp > health.hp)
            {
                addHP(maxHp - health.hp);
            }

            const int32 maxMp = GetMaxMP();
            if (maxMp > health.mp)
            {
                addMP(maxMp - health.mp);
            }
...
auto CCharEntity::OnItemFinish(CItemState& state, action_t& action) -> bool
{
    ...
    // Scroll of Instant Warp (4181): utility item — not consumed on use (no reuse delay).
    if (PItem->getID() == 4181u && PItem->isType(ITEM_USABLE))
    {
...
void CCharEntity::Raise()
{
    ...
    // Same tick as packet handler: zone spawn/aggro can run before Lua timers fire (e.g. DETECT_LOWHP / true sight).
    luautils::OnPlayerRaiseAccept(this);
}
